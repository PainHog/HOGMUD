...
char *	const		sect_names[SECT_MAX][2] =
{
    ...
    { "In the lava", "lava"     },	{ "In the swamp", "swamp"	},
    { "On a boat", "boat"       }
};
...
ch_ret move_char( CHAR_DATA *ch, EXIT_DATA *pexit, int fall, bool running )
{
    ...
    { 
        ...
        {
	   send_to_char( "&CYou splash about in the water.&w\n\r", ch );
	}
    }

    if ( IS_NPC(ch) && xIS_SET(ch->act, ACT_AQUATIC) )
    { 
        if(to_room->sector_type != SECT_WATER_NOSWIM &&
           to_room->sector_type != SECT_WATER_SWIM  &&
	   to_room->sector_type != SECT_UNDERWATER)
        { 
             send_to_char( "Aquatic flagged mobs must remain in water.\n\r", ch );
             return rSTOP;
        }    
    }

    if ( IS_NPC(ch) && xIS_SET(ch->act, ACT_BOAT) )
    {   
        if(to_room->sector_type != SECT_WATER_NOSWIM &&
           to_room->sector_type != SECT_WATER_SWIM)
        {
             send_to_char( "Boat flagged mobs must remain on the surface of water.\n\r", ch );
             return rSTOP;
        }     
    } 
...
/* Pretty sure we don't need to check for guilds anymore now that we have
   the new dh. -- Narn   Place these under the next 4 lines of next function.
*/
/*
...
*/
	...
	if ( IS_SET( pexit->exit_info, EX_CLIMB ) )
	{
	    ...
	    if ( !found && !ch->mount )
	    {
		if ( ( !IS_NPC(ch) && (number_percent( ) + 5) > LEARNED(ch, gsn_climb) )
	...
	if(!IS_NPC(ch)) call_lua_num (ch, "entered_room", ch->in_room->vnum);
...
void do_run( CHAR_DATA *ch, char *argument )
{
    ...
    if( IS_SET(ch->in_room->room_flags, ROOM_NORUN))
    {
	send_to_char( "There isn't enough room to run about here.\n\r", ch);
        return;
    }
...
void do_bashdoor( CHAR_DATA *ch, char *argument )
{
	...
	if ( ( pexit = find_door( ch, arg, FALSE ) ) != NULL )
	{
	    ...
	    if ( !IS_NPC(ch) )
		chance = LEARNED(ch, gsn_bashdoor);
	    else
		chance = 75;
	    if ( IS_SET( pexit->exit_info, EX_LOCKED ) )
		chance -= 40;
		...
		damage( ch, ch, ( ch->max_hit / 10 ), gsn_bashdoor );
		...
		damage( ch, ch, ( ch->max_hit / 10 ) + 10, gsn_bashdoor );
