...
OBJ_DATA *save_mob_equipment[MAX_WEAR][MAX_LAYERS];
...
void de_equip_char( CHAR_DATA *ch )
{
	...
	{
	    if ( get_trust( ch ) >= 0 )   //Changed by Drazuk to allow anyone to wear any equ
	    {
		for ( x = 0; x < MAX_LAYERS; x++ )
        {
            if(!SEQ(ch, obj->wear_loc, x))
            {
                SET_SEQ(ch, obj->wear_loc, x, obj);
                break;
            }
        }
		if ( x == MAX_LAYERS )
...
void save_char_obj( CHAR_DATA *ch )
{
	...
	if(!IS_NPC(ch)) call_lua (ch, "saving", NULL);
...
void fwrite_char( CHAR_DATA *ch, FILE *fp )
{
	...
	if( !ch->pcdata->pc_id )
	{
		...
		ch->pcdata->pc_id = sysdata.PlayerIndex;
	}
    ...
    fprintf( fp, "Mobmaster    %d\n",   ch->mobmastercomplete   );
    fprintf( fp, "MobMTicks    %d\n",   ch->mobmticks           );
    fprintf( fp, "NextMobM     %d\n",   ch->nextmobm            );
    fprintf( fp, "Glory        %d\n",   ch->pcdata->quest_curr  );
    fprintf( fp, "MGlory       %d\n",   ch->pcdata->quest_accum );
    fprintf( fp, "Scoremobm    %d\n",   ch->pcdata->scoremobm   );
    fprintf( fp, "Scoremk      %d\n",   ch->pcdata->scoremk		);
    fprintf( fp, "Scoreqp      %d\n",   ch->pcdata->scoreqp     );
    fprintf( fp, "Scorepk      %d\n",   ch->pcdata->scorepk     );
    fprintf( fp, "Scoregold    %d\n",   ch->pcdata->scoregold   );
	fprintf( fp, "Scorexp      %d\n",	ch->pcdata->scorexp		);
	fprintf( fp, "Scoremp      %d\n",	ch->pcdata->scoremp		);
	fprintf( fp, "Scoresp      %d\n",	ch->pcdata->scoresp		);
	fprintf( fp, "SkillPoints  %d\n",	ch->pcdata->skillpoints );
	fprintf( fp, "MagicPoints  %d\n",	ch->pcdata->magicpoints );
    if( ch->pcdata->spouse )
    {
      fprintf( fp, "Spouse        %s~\n", ch->pcdata->spouse    );
    }
    //~ if( ch->pcdata->house_vnum )
    //~ {
      //~ fprintf( fp, "House	  %d\n", ch->pcdata->house_vnum );
    //~ }
    if( ch->pcdata->mobdeathvnum )
    {
	fprintf( fp, "MobDeathVnum	%d\n", ch->pcdata->mobdeathvnum );
    }
...
void fwrite_obj( CHAR_DATA *ch, OBJ_DATA *obj, FILE *fp, int iNest, sh_int os_type, bool hotboot )
{
    ...
    /*    Munch magic flagged containers for now - bandaid      WHY? - Drazuk*/
    ...
    if ( obj->cost != obj->pIndexData->cost )
      fprintf( fp, "Cost         %d\n",	obj->cost		     );
    fprintf( fp, "Values       %d ", obj->value[0]);
    int i;
    for(i=1; i<15;i++)
        fprintf( fp, "%d%s", obj->value[i], i == 14? "\n": " "  );
...
void fread_char( CHAR_DATA *ch, FILE *fp, bool preload, bool copyover )
{
	...
	case 'A':
	    ...
	    KEY( "ArmorWorn",   ch->armorworn,		fread_number( fp ) );
            KEY( "Autorent",	bogus,	fread_number( fp ) );
...
*/
	...
	case 'D':
	    ...
	    KEY( "Description",	ch->description,	fread_string( fp ) );
            KEY( "Dual", ch->dualclass,			fread_number( fp ) );
	    break;
	...
	case 'H':
	    ...
	    KEY( "Homepage",	bogusbuf,	fread_string_nohash( fp ) );
            KEY( "House",       bogus, fread_number( fp ) );
	...
	case 'I':
		KEY( "ID",		ch->pcdata->pc_id, fread_number( fp ) );
	    ...
	    KEY ( "IMC",	bogus,	fread_number( fp ) );
	    KEY ( "IMCAllow",	bogus,	fread_number( fp ) );
	    KEY ( "IMCDeny",	bogus,	fread_number( fp ) );
	    KEY ( "ICEListen",	bogusbuf, fread_string_nohash( fp ) );
	...
	case 'M':
		KEY( "MagicPoints",		ch->pcdata->magicpoints,	fread_number( fp ) );
	    ...
	    KEY( "MobDeathVnum", ch->pcdata->mobdeathvnum, fread_number( fp ) );
	    KEY( "Mobinvis",	ch->mobinvis,		fread_number( fp ) );
            KEY( "Mobmaster",	ch->mobmastercomplete,	fread_number( fp ) );
	    KEY( "MobMTicks",	ch->mobmticks,		fread_number( fp ) );
	    if ( !strcmp( word, "MobRange" ) )
	    {
		bogus = fread_number( fp );
		bogus = fread_number( fp );
		fMatch = TRUE;
	    }
	    break;
	...
	case 'N':
	    KEY ("Name", ch->name, fread_string( fp ) );
            KEY( "NextMobM",	ch->nextmobm,		fread_number( fp ) );
            KEY( "Norares",	bogus,	fread_number( fp ) );
			...
			KEY ("NextMquest", ch->pcdata->nextmquest, fread_number( fp ));
			KEY ("NextSquest", ch->pcdata->nextsquest, fread_number( fp ));
	...
	case 'S':
        KEY( "Scoregold",   ch->pcdata->scoregold,  fread_number( fp ) );
	    KEY( "Scoremk",	ch->pcdata->scoremk,	fread_number( fp ) );
        KEY( "Scoremobm",   ch->pcdata->scoremobm,  fread_number( fp ) );
        KEY( "Scorepk",     ch->pcdata->scorepk,    fread_number( fp ) );
        KEY( "Scoreqp",     ch->pcdata->scoreqp,    fread_number( fp ) );
		KEY( "Scorexp",		ch->pcdata->scorexp,	fread_number( fp ) );
		KEY( "Scoremp",		ch->pcdata->scoremp,	fread_number( fp ) );
		KEY( "Scoresp",		ch->pcdata->scoresp,	fread_number( fp ) );
	    KEY( "Sex",		ch->sex,		fread_number( fp ) );
	    KEY( "ShortDescr",	ch->short_descr,	fread_string( fp ) );
		KEY( "SkillPoints",	ch->pcdata->skillpoints,	fread_number( fp ) );
		...
		ch->saving_damage       = ch->saving_wand;
	    ...
	    {
		ch->saving_damage       = fread_number( fp );
		ch->saving_wand 	= fread_number( fp );
	...
	case 'T':
	    ...
	    KEY( "Tierexp",		ch->tierexp,		fread_number( fp ) );
...
void fread_obj( CHAR_DATA *ch, FILE *fp, sh_int os_type )
{
    ...
    if ( ch )
   {
		...
		else
		{
			...
			&&   wear_loc < MAX_WEAR )
			{
			   int x;
			    ...
			    for ( x = 0; x < MAX_LAYERS; x++ )
                {
                    if(!SEQ(ch, wear_loc, x))
                    {
                        SET_SEQ(ch, wear_loc, x, obj);
                        slot = x;
                        reslot = TRUE;
                        break;
                    }
	...
	case 'L':
	    KEY( "Level",	bogus,		fread_number( fp ) );
	    break;
  ...
  for ( ; ;) {
	...
	case 'E':
 	...
 	case 'F':
        ...
        break;
    case 'H':
        KEY("Hit", mob->hit, fread_number( fp ));
        break;
	case 'L':
		...
		break;
    case 'M':
        KEY( "MaxHit", mob->max_hit, fread_number( fp ));
        break;
	case 'N':
