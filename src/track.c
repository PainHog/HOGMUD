...
void do_track( CHAR_DATA *ch, char *argument )
{
    ...
    {
	send_to_char("&GYou can't find a trail of anyone like that.&w\n\r", ch);
	if (xIS_SET(ch->act, PLR_STALK) )
	{
    ...
    {
	case BFS_ERROR:
	    send_to_char("&G\n\rHmm... something seems to be wrong.&w\n\r", ch);
	    if (xIS_SET(ch->act, PLR_STALK))
            {
               xREMOVE_BIT(ch->act, PLR_STALK);
            }	
	...
	case BFS_ALREADY_THERE:
	    send_to_char("&G\n\rYou're already in the same room!&w\n\r", ch);
            if( xIS_SET(ch->act, PLR_STALK) )
	    {
		xREMOVE_BIT( ch->act, PLR_STALK );
 	    }
	    break;
	case BFS_NO_PATH:
	    send_to_char("&G\n\rYou can't sense a trail from here.&w\n\r", ch);
	    if (xIS_SET(ch->act, PLR_STALK))
            {
               xREMOVE_BIT(ch->act, PLR_STALK);
            }
	    ...
	    break;
	default:
	    ch_printf(ch, "&G\n\rYou sense a trail %s from here...&w\n\r", dir_name[dir]);
...
{   
    if ( IS_NPC(ch) )
    {
	send_to_char( "Sorry, Mobs can't stalk!.\n\r", ch);
	return;
    }
    if ( !IS_NPC(ch) && ch->pcdata->learned[gsn_stalk] <= 0 )
    {
        send_to_char("You do not know of this skill yet.\n\r", ch );
        return;
    }  
    ...
    if ( !IS_NPC(ch) && ch->pcdata->learned[gsn_track] <= 0 )
    {
        send_to_char("You cannot stalk unless you know how to track!\n\r", ch );
        return;
    }
 
    if ( argument[0]=='\0' )
    {
        send_to_char("Whom are you trying to stalk?  Type 'stalk clear' to stop stalking.\n\r", ch);
        return;
    }

    if ( !str_cmp( argument, "clear" ) )
    {
        send_to_char( "You stop stalking your prey.\n\r", ch);
	xREMOVE_BIT( ch->act, PLR_STALK );
	return;
    }
    ...
    if (!can_use_skill( ch, number_percent(), gsn_stalk ) )
    {
        send_to_char( "You fail to find any signs of your prey.\n\r", ch);
        learn_from_failure(ch, gsn_stalk);
        return;
    }

    ch->pcdata->stalking = argument;
    xSET_BIT( ch->act, PLR_STALK );
    send_to_char( "You begin to follow the signs left by your prey.\n\r", ch);
