...
/* Aurora's Mortality Command -- Drazuk */
void do_mortality( CHAR_DATA *ch, char *argument )
{
    ...
    if ( arg[0] == '\0' && (!IS_IMMORTAL(ch) && ch->tmplevel < 52))
    {
	send_to_char( "Huh?\n\r", ch );
	return;
    }
	...
	send_to_char( "Invalid Level.\n\r", ch );
     ...
     if (!str_cmp(argument, "broadcast") || !str_cmp(arg, "broadcast") || ch->tmplevel < 60)
     {
      //~ sprintf(buf, "%s has taken on a Mortal avatar within the realms.", ch->name);
         call_va(L_mud, "sprintf", "ss>s", "%s has taken on a Mortal avatar within the realm.", ch->name);
      echo_to_all(AT_IMMORT, buf, ECHOTAR_ALL);  
     }   
     return;
    }
    if (!IS_IMMORTAL(ch) && ch->tmplevel > 50)
    {
    ...
    ch->tmptrust = 0;
    save_char_obj(ch);
    act( AT_PLAIN, "$n returns to an immortal state.", ch, NULL, NULL, TO_ROOM );
    act( AT_PLAIN, "You return to your immortal state.", ch, NULL, NULL, TO_CHAR );
     if (!str_cmp(argument, "broadcast") || !str_cmp(arg, "broadcast") || ch->level < 60)
     {
      //~ sprintf(buf, "%s ascends into the heavens once more.", ch->name);
         call_va(L_mud, "sprintf", "ss>s", "%s ascends into the heavens once more.", ch->name);
      echo_to_all(AT_IMMORT, buf, ECHOTAR_ALL);  
     }   
    return;
    }
...
void do_invade( CHAR_DATA *ch , char *argument )
{
    ...
    call_va(L_mud, "one_arg", "s>ss", argument, &arg3, &argument);
    call_va(L_mud, "one_arg", "s>ss", argument, &arg4, &argument);
    
    count = atoi( arg2 );
	glory = atoi( arg4 );
    if ( arg1[0] == '\0' || arg2[0] == '\0' )
    {
	send_to_char( "Invade <area> <# of invaders> <mob vnum> <glory awarded>\n\r", ch );
	return;
    }
	...
	{
		send_to_char("Glory range from 0 to 150.\n\r", ch);
		return;
	}
    ...
    call_va(L_mud, "sprintf", "ss>s", "&WThe invasion of &C%s&W was successful!&D\n\r", tarea->name, &arg1);
  ...
  if ( arg2[0]=='\0' || !str_cmp( arg2,"accept" ) || !str_cmp( arg2,"yes" ))
     {
        ...
        to_channel( buf, CHANNEL_AUTH, "Auth", LEVEL_MASTER );
...
void do_fixcharmie( CHAR_DATA *ch, char *argument )
{
    char arg1[MAX_INPUT_LENGTH];
    char arg2[MAX_INPUT_LENGTH];
    CHAR_DATA *victim;
    ...
    if ( arg1[0] == '\0' || arg2[0] == '\0' )
    {
	send_to_char( "Syntax: fixcharmie <charmie> <character>.\n\r", ch );
	return;
    }
    if ( ( charmie = get_char_room( ch, arg1 ) ) == NULL )
    {
	send_to_char( "That charmie isn't here.\n\r", ch );
	return;
    }

    if ( ( victim = get_char_room( ch, arg2 ) ) == NULL )
    {
	send_to_char( "The character isn't here.\n\r", ch );
	return;
    }
    ...
    {
	send_to_char( "The charm must of worn off that mob.\n\r", ch );
	return;
    }

    add_follower( charmie, victim );
    if ( xIS_SET( charmie->act, ACT_MOUNTED ) )
    {
	xREMOVE_BIT( charmie->act, ACT_MOUNTED );
    }

    send_to_char( "Fixed Charmie.\n\r", ch );
    return;
}
...
void do_pardon( CHAR_DATA *ch, char *argument )
{
    ...
    if ( arg1[0] == '\0' || arg2[0] == '\0' )
    {
	send_to_char( "Syntax: pardon <character> <killer|thief|bully>.\n\r", ch );
	return;
    }
    ...
    if ( ch->mana < 1000 )
    {
	send_to_char( "You need at least 1000 mana to pardon a character.\n\r", ch );
	return;
    }

    if ( !str_cmp( arg2, "bully" ) )
    {
	if ( xIS_SET(victim->act, PLR_BULLY) )
	{
	    xREMOVE_BIT( victim->act, PLR_BULLY );
	    ch_printf( ch, "Bully flag removed from %s.\n\r", victim->name );
	    set_char_color( AT_IMMORT, victim );
    ...
    send_to_char( "Syntax: pardon <character> <killer|bully|thief>.\n\r", ch );
    return;
}
...
void echo_to_area( sh_int AT_COLOR, CHAR_DATA *ch, char *argument )
{
    CHAR_DATA *vic;
    CHAR_DATA *vic_next;

    for ( vic = first_char; vic; vic = vic_next )
    {
        vic_next = vic->next;
        if ( vic->in_room->area == ch->in_room->area 
        &&  !IS_NPC(vic)
        &&   IS_AWAKE(vic) )
        {
            set_char_color( AT_COLOR, vic );
...
void do_rwho( CHAR_DATA *ch, char *argument )
{
    ...
    send_to_char_color( "&cVisible Characters: &w", ch );
...
void do_rstat( CHAR_DATA *ch, char *argument )
{
	...
	//~ {
		//~ extern char * const mine_name[];
		//~ extern char * const gather_name[];

		//~ send_to_char_color( "&c------------------- &wJOB DATA &c----------------\n\r", ch );
		//~ send_to_char_color( "&cMine or Gather                  Quantity remaining\n\r", ch );
		//~ for( cnt= 0; cnt < 10; cnt++)
		//~ {
			//~ ch_printf( ch,  "Item %-2d: %-20s     %d\n\r", cnt, 
				//~ (location->sector_type == SECT_MOUNTAIN || location->sector_type == SECT_UNDERGROUND)?mine_name[cnt]:gather_name[cnt],
				//~ location->job_items[cnt] );
		//~ }
	//~ }
    return;
}

void do_wizmark ( CHAR_DATA *ch, char *argument )
{
    char arg[MAX_INPUT_LENGTH];
    OBJ_DATA *obj;
...
/* retrieve an item from anywhere using its id number -Darwin
 * Syntax: oget <id> 
 * or    : oget v<vnum>
 */
void do_oget( CHAR_DATA *ch, char *argument )
{
	OBJ_DATA *obj;
	int value = -1;
	bool vnum = FALSE;
	...
	if(!found)
	{
		send_to_char("&WCould not find that item.\n\r", ch);
		return;
	}
	else
	{
...
void do_ostat( CHAR_DATA *ch, char *argument )
{
    ...
    ch_printf_color( ch, "&cName: &C%-40s ID: %d\n\r", obj->name, obj->obj_id );
	...
	ch_printf_color( ch, "&w%d\n\r", obj->timer );
    //~ ch_printf_color( ch, "&cLevel: &P%d\n\r", obj->level );
    if ( obj->owner )
	ch_printf_color( ch, "&cOwner:  &w%s\n\r", obj->owner );
    ...
    send_to_char( "&cIndex values : &w", ch);
    int i;
    for(i = 0; i < 15; i++)
        ch_printf_color( ch, "%d ", obj->pIndexData->value[i]);
    send_to_char("\n\r&cObject Values: &w", ch);
    for(i = 0; i < 15; i++)
        ch_printf_color( ch, "%d ", obj->value[i]);
    send_to_char("&D\n\r", ch);
...
void do_mstat( CHAR_DATA *ch, char *argument )
{
	...
	if(IS_NPC(victim))
		ch_printf( ch, "\n\r&cMobile name: &C%-40s &cID: &C%d", victim->name, victim->mob_id );
	else
	    ch_printf( ch, "\n\r&cName: &C%-20s", victim->name );
    if ( !IS_NPC(victim) )
		ch_printf( ch, "&cStatus : &w%-10s", CAN_PKILL(victim) ? "Deadly" : IS_PKILL(victim) ? "Pre-Deadly" : "Non-Deadly" );
    ...
    if ( get_trust(ch) >= LEVEL_GOD && !IS_NPC(victim) && victim->desc )
		ch_printf( ch, "&cIP: &w%s   Descriptor: %d  &cTrust: &w%d\n\r", victim->desc->host, victim->desc->descriptor, victim->trust);
    if ( !IS_NPC(victim) && victim->pcdata->release_date != 0 )
		ch_printf( ch, "&cHelled until %24.24s by %s.\n\r",	ctime(&victim->pcdata->release_date), victim->pcdata->helled_by);
	if(!IS_NPC(victim))
		ch_printf( ch, "&cID  : &w%-5d", victim->pcdata->pc_id );
	else
		ch_printf( ch, "&cVnum: &w%-5d", victim->pIndexData->vnum );
    ch_printf( ch, "    &cSex: &w%-6s    &cRoom: &w%-5d    &cCount: &w%d   &cKilled: &w%d\n\r",
		get_sex(victim), victim->in_room == NULL ? 0 : victim->in_room->vnum,
    ...
    ch_printf( ch, "&cStr: &C%2d&c(&C%2d&c) Int: &C%2d&c(&C%2d&c) Wis: &C%2d&c(&C%2d&c) Dex: &C%2d&c(&C%2d&c) Con: &C%2d&c(&C%2d&c) Cha: &C%2d&c(&C%2d&c) Lck: &C%2d&c(&C%2d&c)\n\r",
		get_curr_str(victim), victim->perm_str,
		get_curr_int(victim), victim->perm_int,
		get_curr_wis(victim), victim->perm_wis,
		get_curr_dex(victim), victim->perm_dex,
		get_curr_con(victim), victim->perm_con,
		get_curr_cha(victim), victim->perm_cha,
		get_curr_lck(victim), victim->perm_lck );
    ch_printf( ch, "&cLevel   : &P%-2d              ", victim->level );
    ch_printf( ch, "&cClass  : &w%-2.2d/%-10s  &cDual : &w%-2.2d/%-10s  &cRace : &w%-2.2d/%-10s\n\r",
		victim->class,
        ...
        victim->dualclass,
		IS_NPC(victim)?victim->dualclass<MAX_NPC_CLASS&&victim->dualclass>=0?
        npc_class[victim->dualclass] : "unknown":victim->dualclass<MAX_PC_CLASS&&victim->dualclass>=0&&
	...
	ch_printf( ch, "&cExp     : &C%-10d      &cTNL    : &C%-10d     &cPrac : &C%d&c\n\r", victim->exp, exp_level(victim, victim->level+1) - victim->exp, victim->practice);
    ch_printf( ch, "&cHps     : &W%5d&w[&R%5d&W]    &cMana   : &W%5d&w[&R%5d&W]    &cMove      : &W%5d&w[&R%5d&W]\n\r",
		victim->hit, victim->max_hit, victim->mana, victim->max_mana, victim->move, victim->max_move );
    ch_printf( ch,  "&cHitroll : &C%-5d           &cAlign  : &w%-5d           &cArmorClass: &w%d\n\r",
		GET_HITROLL(victim), victim->alignment, GET_AC(victim) );
    ch_printf( ch, "&cDamroll : &C%-5d           &cWimpy  : &w%-5d           &cPosition  : &w%s\n\r",
		GET_DAMROLL(victim), victim->wimpy, get_position_string(victim) );
    ch_printf( ch, "&cFighting: &w%-13s   &cMaster : &w%-13s   &cLeader    : &w%s\n\r",
		victim->fighting ? victim->fighting->who->name : "(none)",
        victim->master      ? victim->master->name   : "(none)",
        victim->leader      ? victim->leader->name   : "(none)" );
    if ( IS_NPC(victim) )
		ch_printf( ch, "&cHating  : &w%-13s   &cHunting: &w%-13s   &cFearing   : &w%s\n\r",
		victim->hating ? victim->hating->name : "(none)",
		victim->hunting ? victim->hunting->name : "(none)",
        victim->fearing ? victim->fearing->name : "(none)" );
    else
		ch_printf( ch, "&cDeity   : &w%-13s&w   &cFavor  : &w%-5d           &cGlory     : &w%-d (%d)\n\r",
		victim->pcdata->deity ? victim->pcdata->deity->name : "(none)",
        victim->pcdata->favor,
        victim->pcdata->quest_curr,
        victim->pcdata->quest_accum );
    if ( IS_NPC( victim ) )
		ch_printf( ch, "&cMob hitdie : &C%dd%d+%d    &cMob damdie : &C%dd%d+%d+%d    &cNumAttacks : &C%d\n\r",
        ...
        victim->damroll,
		victim->numattacks );
    ch_printf( ch, "&cMentalState: &w%-3d   &cEmotionalState: &w%-3d   ", victim->mental_state, victim->emotional_state );
    if ( !IS_NPC( victim ) )
		ch_printf( ch, "&cThirst: &w%d   &cFull: &w%d   &cDrunk: &w%d\n\r",
		victim->pcdata->condition[COND_THIRST],
		victim->pcdata->condition[COND_FULL],
		victim->pcdata->condition[COND_DRUNK] );
    else
	{
		if(victim->charmed_from)
			ch_printf(ch, "&cCharmed from room: &w%-6d", victim->charmed_from);
		ch_printf(ch, "\n\r");
	}
	if(!IS_NPC(victim))
		ch_printf(ch, "&cSkill points: &w%5d&W(%d)     &cMagic points: &w%5d&W(%d)&D\n\r", 
		victim->pcdata->skillpoints, (int)victim->pcdata->skillpoints/350, 
		victim->pcdata->magicpoints, (int)victim->pcdata->magicpoints/350);
    ch_printf( ch, "&cSave versus: &w%d %d %d %d       &cItems: &w(%d/%d)  &cWeight &w(%d/%d)\n\r",
		victim->saving_damage,
    ...
    if ( !IS_NPC(victim) && victim->pcdata->charmies)
        ch_printf( ch, "&cCharmies: &w%d\n\r", victim->pcdata->charmies );

    if ( !IS_NPC(victim) && victim->pcdata->mobdeathvnum )
		ch_printf( ch, "&cLast NPC Killed By: &w%d\n\r", victim->pcdata->mobdeathvnum );
    
    ch_printf( ch, "&cSpiritshield: &w%d      &cSpell Damage: &w%d      &cWorn AC: &w%d\n\r", 
		victim->spiritshield, GET_SPELLDAM(victim), victim->armorworn );

    if ( !IS_NPC(victim) && victim->pcdata->pkvictim)
		ch_printf( ch, "&cPK Victim: &w%s\n\r", victim->pcdata->pkvictim );

    ch_printf( ch, "&cYear: &w%-5d  &cSecs: &w%d  &cTimer: &w%d  &cGold: &Y%d",	get_age( victim ), (int) victim->played, victim->timer, victim->gold );
    if(IS_NPC(victim))
        ch_printf( ch, "  &cGlory award: &w%d\n\r", victim->glory_award);
    else
        ch_printf( ch, "\n\r");

    if ( get_timer( victim, TIMER_PKILLED ) )
		ch_printf( ch, "&cTimerPkilled:  &R%d\n\r", get_timer( victim, TIMER_PKILLED ) );
    if ( get_timer( victim, TIMER_RECENTFIGHT ) )
		ch_printf( ch, "&cTimerRecentfight:  &R%d\n\r", get_timer( victim, TIMER_RECENTFIGHT ) );
    if ( get_timer( victim, TIMER_THIEF ) ) 
        ch_printf( ch, "&cTimerThief:  &R%d\n\r", get_timer( victim, TIMER_THIEF ) );
    if ( get_timer( victim, TIMER_ASUPRESSED ) )
		ch_printf( ch, "&cTimerAsupressed:  &R%d\n\r", get_timer( victim, TIMER_ASUPRESSED ) );
    if ( IS_NPC( victim ) )
		ch_printf( ch, "&cAct Flags  : &w%s\n\r", ext_flag_string(&victim->act, act_flags) );
    else
    {
		ch_printf( ch, "&cPlayerFlags: &w%s\n\r", ext_flag_string( &victim->act, plr_flags ) );
		ch_printf( ch, "&cPcflags    : &w%s\n\r", flag_string( victim->pcdata->flags, pc_flags ) );
		if ( victim->pcdata->nuisance  )
			ch_printf( ch, "&RNuisance   &cStage: (&R%d&c/%d)  Power:  &w%d  &cTime:  &w%s.\n\r", victim->pcdata->nuisance->flags, 
				MAX_NUISANCE_STAGE, victim->pcdata->nuisance->power, ctime(&victim->pcdata->nuisance->time));
    }
    if ( victim->morph )
    {
		if ( victim->morph->morph )
			ch_printf(ch, "&cMorphed as : (&C%d&c) &C%s    &cTimer: &C%d\n\r", victim->morph->morph->vnum, victim->morph->morph->short_desc, victim->morph->timer );
		else
			ch_printf (ch, "&cMorphed as: Morph was deleted.\n\r" );
    }
    ch_printf( ch, "&cAffected by: &C%s\n\r", affect_bit_name( &victim->affected_by ) );
    ch_printf( ch, "&cSpeaks: &w%d   &cSpeaking: &w%d", victim->speaks, victim->speaking );
    if ( !IS_NPC( victim ) && victim->wait )
		ch_printf( ch, "   &cWaitState: &R%d\n\r", victim->wait/12 );
    else
	ch_printf( ch, "\n\r" );
    ch_printf( ch, "&cLanguages  : &w" );
    ...
    if ( victim->pcdata && victim->pcdata->bestowments && victim->pcdata->bestowments[0] != '\0' )
		ch_printf( ch, "&cBestowments: &w%s\n\r", victim->pcdata->bestowments );
    if ( IS_NPC( victim ) )
		ch_printf( ch, "&cShortdesc  : &w%s\n\r&cLongdesc   : &w%s",
...
void do_snoop( CHAR_DATA *ch, char *argument )
{
     ...
     */
    if ( (get_trust(ch) < LEVEL_SUPREME && get_trust( victim ) >= LEVEL_CREATOR)
...
void do_oinvoke( CHAR_DATA *ch, char *argument )
{
    ...
    ch_printf_color( ch, "&YYou invoke %s (&W#%d &Y- &W%s&Y)&D\n\r",
  ...
  /*You can demote yourself but not someone else at your own trust.-- Narn*/
        ...
        else
	{
	  ...
	  send_to_char( "Deja vu!  Your mind reels as you re-live your past levels!\n\r", victim );
        ...
        victim->mana     = victim->max_mana;
        victim->move     = victim->max_move;
        if(xIS_SET(victim->act, PLR_TIER))
        {
          tadvance_level(victim); 
        }
        else
	{
	  advance_level( victim );
...
void do_force( CHAR_DATA *ch, char *argument )
{
	...
	if(ch->level < 65) // allow imps to force imps. could be bad for some muds -Darwin
	if ( ( get_trust( victim ) >= get_trust( ch ) ) 
...
void do_newbieset( CHAR_DATA *ch, char *argument )
{
	...
	return;
     ...
     if ( (victim->class == CLASS_MAGE) || (victim->class == CLASS_THIEF) ||
	  (victim->class == CLASS_DIABOLIST) || (victim->class == CLASS_CONJURER) )
     {
     ...
     {
        obj = create_object(get_obj_index(OBJ_VNUM_SCHOOL_CLAW), 1 );
	obj_to_char(obj, victim);
     }
     else if (victim->class == CLASS_THUG )
     {
        obj = create_object(get_obj_index(OBJ_VNUM_SCHOOL_CLUB), 1 );
        obj_to_char(obj, victim);
     }
...
/*
void do_grantsight( CHAR_DATA *ch, char *argument ) 
{ 
    char arg[MAX_INPUT_LENGTH];
    CHAR_DATA *victim;
    ...
    if ( arg[0] == '\0' )
    { 
        send_to_char( "Grant which player Godsight??\n\r", ch );
        return;
    }    
    if ( ( victim = get_char_world( ch, arg ) ) == NULL )
    { 
    ...
    if ( IS_NPC( victim ) ) 
    { 
        send_to_char( "You can't give Godsight to a mob!\n\r", ch );
        return; 
    } 

    if ( ch->pcdata->clan == NULL)
    {
	send_to_char( "That player does not have a Following Deity to see!\n\r", ch );
	return;
    }

    if (!xIS_SET(victim->act, PLR_GODSIGHT))
    { 
       set_char_color( AT_IMMORT, victim );
       xSET_BIT(victim->act, PLR_GODSIGHT);
       ch_printf( victim, "%s has bestowed Godsight on you.\n\r", ch->name );
       send_to_char( "Done.\n\r", ch );
    }
    else
    {
	set_char_color( AT_IMMORT, victim );
	xREMOVE_BIT(victim->act, PLR_GODSIGHT);
	ch_printf( victim, "%s has removed your Godsight.\n\r", ch->name );
...
/*
 *  new grantsight. allows multiple immortals to grantsight the same mortal -Darwin
 */
void do_grantsight( CHAR_DATA *ch, char *argument ) 
{ 
    char arg[MAX_INPUT_LENGTH];
	...
	for(i = 0; i < 10; i++) // search array for ch name
	{
		if(victim->gs_by[i])
			if(!strcmp(victim->gs_by[i], ch->name)) // if found, remove ch name
			{
				STRFREE(victim->gs_by[i]);
				pager_printf_color( ch, "&WGS removed from %s.\n\r", victim->name);
				pager_printf_color( victim, "&W%s has removed your grantsight.\n\r", ch->name);
				return;
			}
	}
	// if we make it this far, name wasn't found in array, so add it to an empty slot
	for(i = 0; i < 10; i++)
	{
		if(!victim->gs_by[i])
		{
			//add here
			victim->gs_by[i] = STRALLOC(ch->name);
			pager_printf_color(ch, "&W%s has been grantsighted.\n\r", victim->name);
			pager_printf_color(victim, "&W%s has grantsighted you.\n\r", ch->name);
			return;
		}
	}
	// if we make it this far, the array is full
	pager_printf_color(ch, "&W%s cannot recieve any more grantsights.\n\r", victim->name);
	return;
}
...
void do_destroy( CHAR_DATA *ch, char *argument )
{
  ...
  sprintf( buf3, "%s%s", LOCKER_DIR, name );
  sprintf( buf4, "%sbackup/%s", LOCKER_DIR, name );

    if ( !rename( buf3, buf4) )
    {
		ch_printf(ch, "Player %s's locker moved to backup directory.\n\r", name);
    }
...
void do_cset( CHAR_DATA *ch, char *argument )
{
  ...
  { 
    ...
    pager_printf_color(ch, "&wNewbie Lock: &W%s\n\r",
	    sysdata.DENY_NEW_PLAYERS ? "Enabled" : "Disabled" );
  ...
  if ( !str_cmp( arg, "newbie" ) )
  {
	...
	send_to_char( "New characters halted.\n\r", ch );
     else
	send_to_char( "New characters allowed.\n\r", ch );
     return;
  }
...
void do_showclass( CHAR_DATA *ch, char *argument )
{
    ...
    pager_printf_color( ch, "&wSecond Attribute:  &W%-14s  &wTertiary Attribute:  &W%-14s\n\r",
    	affect_loc_name(class->attr_second), affect_loc_name(class->attr_tertiary) );
    pager_printf_color( ch, "&wMax Skill Adept: &W%-3d         &wThac0base : &W%-5d   &wThac0mod: &W%d\n\r",
	class->skill_adept, class->thac0base, class->thac0mod );
...
bool create_new_class( int index, char *argument )
{
  ...
  class_table[index]->attr_tertiary = 0;
  ...
  class_table[index]->thac0base = 0;
  class_table[index]->thac0mod = 0;
...
void do_setclass( CHAR_DATA *ch, char *argument )
{
	...
	send_to_char( "\n\rField being one of:\n\r",			ch );
	send_to_char( "  name prime weapon guild thac0base thac0mod\n\r", ch );
	send_to_char( "  hpmin hpmax mana expbase mtitle ftitle\n\r", ch );
	send_to_char( "  second, tertiary affected resist suscept\n\r", ch );
	return;
    }
    if ( is_number(arg1) && (cl=atoi(arg1)) >= 0 && cl < MAX_CLASS )
	class = class_table[cl];
	...
	if ( x < APPLY_STR || (x > APPLY_CON && x != APPLY_LCK && x != APPLY_CHA) )
	    send_to_char( "Invalid tertiary attribute!\n\r", ch );
	else
	{
	    class->attr_tertiary = x;
	    send_to_char( "Done.\n\r", ch );
	}
	return;
    }
    ...
    if ( !str_cmp( arg2, "thac0base" ) )
    {
	class->thac0base = atoi( argument );
	send_to_char( "Done.\n\r", ch );
	return;
    }
    if ( !str_cmp( arg2, "thac0mod" ) )
    {
	class->thac0mod = atoi( argument );
	send_to_char( "Done.\n\r", ch );
	return;
    }
...
void do_setrace( CHAR_DATA *ch, char *argument )
{
    ...
    if ( arg1[0] == '\0' || arg2[0] == '\0' )
    {
        ...
        send_to_char( "  saving_damage saving_wand      \n\r",     ch );
    ...
    if ( !str_cmp( arg2, "saving_damage" ) )
    {
        race->saving_damage = atoi( argument );
        send_to_char( "Done.\n\r", ch );
        return;
    }
...
void do_spset( CHAR_DATA *ch, char *argument )
{
  char arg[MAX_INPUT_LENGTH];
  char arg2[MAX_INPUT_LENGTH];
  char arg3[MAX_INPUT_LENGTH];
  ...
  if ( IS_NPC(ch) )
  {
    send_to_char( "Cannot spset as an NPC.\n\r", ch );
    return;
  }
  ...
  if ( arg[0] == '\0' || arg2[0] == '\0' || amount <= 0 )
  {
    send_to_char( "Syntax: spset <character> <give/take> <amount>\n\r", ch );
    send_to_char( "Amount must be a positive number greater than 0.\n\r", ch );
    return;
  }
  if ( ( victim = get_char_world( ch, arg ) ) == NULL )
  ...
  if ( IS_NPC( victim ) )
  {
    send_to_char( "Skill points cannot be given to or taken from a mob.\n\r", ch );
    return;
  }
  ...
  if ( nifty_is_name_prefix( arg2, "give" ) )
  {
    ...
    && ( get_trust( ch ) < LEVEL_DEMI ) )
    {
      send_to_char( "You must be a member of the Quest Council to give skill points to a character.\n\r", ch );
      return;
    }
  }
  else if ( nifty_is_name_prefix( arg2, "take" ) )
  ...
  if ( give )
  {
    victim->pcdata->skillpoints += amount;
    ch_printf( victim, "Your skill points have been increased by %d.\n\r", amount );
    ch_printf( ch, "You have increased the skill points of %s by %d.\n\r", victim->name, amount );
  }
  else
  {
    if ( victim->pcdata->skillpoints - amount < 0 )
    {
      ch_printf( ch, "%s does not have %d skill points to take.\n\r", victim->name, amount );
      return;
    }
    else
    {
    victim->pcdata->skillpoints -= amount;
    ch_printf( victim, "Your skill points have been decreased by %d.\n\r", amount );
    ch_printf( ch, "You have decreased the skill points of %s by %d.\n\r", victim->name, amount );
    }
  }
  return;
}
...
void do_mpset( CHAR_DATA *ch, char *argument )
{
  ...
  if ( IS_NPC(ch) )
  {
    send_to_char( "Cannot mpset as an NPC.\n\r", ch );
    return;
  }
  ...
  if ( arg[0] == '\0' || arg2[0] == '\0' || amount <= 0 )
  {
    send_to_char( "Syntax: mpset <character> <give/take> <amount>\n\r", ch );
    send_to_char( "Amount must be a positive number greater than 0.\n\r", ch );
    return;
  }
  if ( ( victim = get_char_world( ch, arg ) ) == NULL )
  ...
  if ( IS_NPC( victim ) )
  {
    send_to_char( "Magic points cannot be given to or taken from a mob.\n\r", ch );
    return;
  }
  ...
  if ( nifty_is_name_prefix( arg2, "give" ) )
  {
    ...
    && ( get_trust( ch ) < LEVEL_DEMI ) )
    {
      send_to_char( "You must be a member of the Quest Council to give magic points to a character.\n\r", ch );
      return;
    }
  }
  else if ( nifty_is_name_prefix( arg2, "take" ) )
  ...
  if ( give )
  {
    victim->pcdata->magicpoints += amount;
    ch_printf( victim, "Your magic points have been increased by %d.\n\r", amount );
    ch_printf( ch, "You have increased the magic points of %s by %d.\n\r", victim->name, amount );
  }
  else
  {
    if ( victim->pcdata->magicpoints - amount < 0 )
    {
      ch_printf( ch, "%s does not have %d magic points to take.\n\r", victim->name, amount );
      return;
    }
    else
    {
    victim->pcdata->magicpoints -= amount;
    ch_printf( victim, "Your magic points have been decreased by %d.\n\r", amount );
    ch_printf( ch, "You have decreased the magic points of %s by %d.\n\r", victim->name, amount );
    }
  }
  return;
}
...
"---|-------------|----------------------|--------------------------|-----------\n\r");
   ...
   if( !str_cmp( arg1, "log" ) )
   {
	 ...
	 editor_desc_printf( ch, "A log note in project '%s', entitled '%s'.", pproject->name ? pproject->name : "(No name)",
...
void do_pcrename( CHAR_DATA *ch, char *argument )
{
	...
	send_to_char("\n\rNote: It is important to surround the names in double quotes.(\")\n\r", ch);
	return;
    }
	...
	/*
	 * rename "owner" on quest equipment before actual name change
	 * This prevents the quest equipment from falling off of the
	 * character being renamed -Darwin
	 */
	for( obj = victim->first_carrying; obj ; obj = obj->next)
	{
		if ( xIS_SET( obj->extra_flags, ITEM_QUEST ) && !str_cmp(victim->name, obj->owner) )
		{
			STRFREE(obj->owner);
			obj->owner = STRALLOC(arg2);
		}
	}
    ...
    if ( remove( oldname ) )
    {
	sprintf(buf, "Error: Couldn't delete file %s in do_pcrename.", oldname);
...
void do_obj_copy(CHAR_DATA *ch, char *argument)
{
    char arg1[MAX_INPUT_LENGTH];
	OBJ_DATA *obj;
...
/* Force a mob (or player) to be a charmie of someone else
 * usage: fcharmie <master> <charmie> <duration>
 * Duration of -1 is more or less permanent unless charmie
 * dies or the charm is dispelled. -Darwin
 * Uses double-quotes around parameters if names are more
 * than one word.
 */
void do_forcecharmie( CHAR_DATA *ch, char *argument)
{
	CHAR_DATA *victim;
	CHAR_DATA *mob;
    AFFECT_DATA af;
	int dur;
	char arg1[MAX_STRING_LENGTH]; // player name
	char arg2[MAX_STRING_LENGTH]; // mob name
	char arg3[MAX_STRING_LENGTH]; // duration

	if(!argument)
	{
		send_to_char("Missing parameters.\n\rUsage: fcharmie <master> <charmie> <duration>\n\r", ch);
		return;
	}
	argument = one_argument3( argument, arg1);
	argument = one_argument3( argument, arg2);
	argument = one_argument3( argument, arg3);
...
/*
 * If economies get out of hand due to repeated reboots,
 * crashes, etc.. then this can be used to reset the
 * economies to the base of 250m if they exceed that.
 * -Darwin
 */
void do_reset_economy( CHAR_DATA *ch, char *argument )
{
    AREA_DATA *tarea;

    for ( tarea = first_area; tarea; tarea = tarea->next )
    {
		tarea->low_economy = 250000000;
		tarea->high_economy = 0;
	}
...
/*
 * Command for immortals to add the random flag to existing
 * non-random items - mainly for clan containers -Darwin
 */
void do_set_random( CHAR_DATA *ch, char *argument )
{
	OBJ_DATA *obj;
	...
	if(!arg1)
	{
		send_to_char("&WWhat do you want to mark as random?\n\r", ch);
		return;
	}
	obj = get_obj_carry(ch, arg1);
	if(!obj)
	{
		send_to_char("&WYou do not have that item.\n\r", ch);
		return;
	}
	...
	if(!xIS_SET(obj->extra_flags, ITEM_RANDOM))
	{
		...
		sprintf(buf, "&WObject '&C%s&W' has been marked as random.\n\r", obj->short_descr);
		send_to_char( buf, ch );
		return;
	}
	else
	{
		sprintf(buf, "&WObject '&C%s&W' is already marked as random.\n\r", obj->short_descr);
		send_to_char(buf, ch);
		return;
	}
}

/* for manually restarting lua mud-state -Darwin 2007-08-10 */
/* ammended 2007-12-05 to allow arguments                   */
void do_restart_lua_mud( CHAR_DATA *ch, char *argument)
{
	char arg[MAX_STRING_LENGTH];
	...
	default:
		...
		{
			close_mud_lua();
			send_to_char("&WClosing global MUD Lua state.\n\r&D", ch);
			add_timer( ch, TIMER_DO_FUN, 3, do_restart_lua_mud, 1 );
		...
		else if(!strcmp(arg, "self"))
		{
			save_char_obj(ch);
			close_lua(ch);
			send_to_char("&WResetting your Lua state...\n\r&D", ch);
			add_timer( ch, TIMER_DO_FUN, 3, do_restart_lua_mud, 1 );
			ch->alloc_ptr = str_dup( arg );
			return;
		}
		else
		{
			...
			if(vch && !IS_NPC(vch))
			{
				save_char_obj(vch);
				close_lua(vch);
				ch_printf(ch, "&WResetting Lua state for %s...\n\r&D", vch->name);
				send_to_char( "&WResetting your Lua state...\n\r&D", vch);
				add_timer( ch, TIMER_DO_FUN, 3, do_restart_lua_mud, 1 );
				ch->alloc_ptr = str_dup( arg );
				return;
				...
				send_to_char("&WTarget not online or could not be found.\n\r&D", ch);
		}
		return;
	case 1:
		if ( !ch->alloc_ptr )
		{
			send_to_char( "Your pointer was lost!\n\r", ch );
			bug( "do_restart_lua_mud: alloc_ptr NULL", 0 );
			return;
		}
		sprintf(arg, "%s", ch->alloc_ptr);
	...
	case SUB_TIMER_DO_ABORT:
		...
		DISPOSE( ch->alloc_ptr );
		ch->substate = SUB_NONE;
		send_to_char( "Skipping the wait...\n\r", ch );
		break;
	}
	...
	{
		open_mud_lua();
		send_to_char("&WLua MUD state reset.\n\r&D", ch);
	}
	else if(!strcmp(arg, "self"))
	{
		open_lua(ch);
		send_to_char("&WYour Lua State has been reset.\n\r&D", ch);
	}
	else
	{
		CHAR_DATA *vch;
...
void do_luascript( CHAR_DATA *ch, char *argument )
{
    if(IS_NPC(ch))
        return;
    
    call_lua(ch, "luascript", argument);
    return;
}
