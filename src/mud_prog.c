...
/* Ifstate defines, used to create and access ifstate array
   in mprog_driver. */
#define MAX_IFS     30		/* should always be generous */
#define IN_IF        0
#define IN_ELSE      1
#define DO_IF        2
#define DO_ELSE      3

#define MAX_PROG_NEST   30
...
doneargs:
    ...
    *p = '\0';
	...
	if ( !str_cmp(chck, "ishero") )
	{
		return (!IS_NPC(chkchar) && IS_HERO(chkchar));
	}
	if ( !str_cmp(chck, "isnohero") )
	{
		return (!IS_NPC(chkchar) && !IS_HERO(chkchar));
	}
	...
	if ( !str_cmp(chck, "isathief") )
	{
	    return ( !IS_NPC(chkchar) && xIS_SET(chkchar->act, PLR_THIEF) );
	}
        if ( !str_cmp(chck, "istiered") )
	{
	    return ( !IS_NPC(chkchar) && xIS_SET(chkchar->act, PLR_TIER) );
	}
        if ( !str_cmp(chck, "isbully") )
	{
	    return ( !IS_NPC(chkchar) && xIS_SET(chkchar->act, PLR_BULLY) );
        }
	...
	{
	    return mprog_veval(chkchar->pcdata->quest_curr, opr, atoi(rval), mob);
	}
...
*/
	...
	if ( !str_cmp(chck, "ismage") )
	{
		return (IS_MAGE(chkchar) || IS_ARCHMAGI(chkchar));
	}
	if ( !str_cmp(chck, "iscleric") )
	{
		return (IS_CLERIC(chkchar) || IS_BISHOP(chkchar));
	}
	if ( !str_cmp(chck, "isthief") )
	{
		return (IS_THIEF(chkchar) || IS_ROGUE(chkchar));
	}
	if ( !str_cmp(chck, "iswarrior") )
	{
		return (IS_WARRIOR(chkchar) || IS_CHAMPION(chkchar));
	}
	if ( !str_cmp(chck, "isthug") )
	{
		return (IS_THUG(chkchar) || IS_ROGUE(chkchar));
	}
	if ( !str_cmp(chck, "isdruid") )
	{
		return (IS_DRUID(chkchar) || IS_ARCHMAGI(chkchar));
	}
	if ( !str_cmp(chck, "isranger") )
	{
		return (IS_RANGER(chkchar) || IS_CHAMPION(chkchar));
	}
	if ( !str_cmp(chck, "ismonk") )
	{
		return (IS_MONK(chkchar) || IS_CHAMPION(chkchar));
	}
	if ( !str_cmp(chck, "isdiabolist") )
	{
		return (IS_DIABOLIST(chkchar) || IS_BISHOP(chkchar));
	}
	if ( !str_cmp(chck, "isconjurer") )
	{
		return (IS_CONJURER(chkchar) || IS_ARCHMAGI(chkchar));
	}
	if ( !str_cmp(chck, "isjester") )
	{
		return (IS_JESTER(chkchar) || IS_ROGUE(chkchar));
	}
	if ( !str_cmp(chck, "isshaman") )
	{
		return (IS_SHAMAN(chkchar) || IS_BISHOP(chkchar));
	}
	if ( !str_cmp(chck, "ischampion") )
	{
		return IS_CHAMPION(chkchar);
	}
	if ( !str_cmp(chck, "isbishop") )
	{
		return IS_BISHOP(chkchar);
	}
	if ( !str_cmp(chck, "isrogue") )
	{
		return IS_ROGUE(chkchar);
	}
	if ( !str_cmp(chck, "isarchmagi") )
	{
		return IS_ARCHMAGI(chkchar);
	}
...
void mprog_look_trigger( CHAR_DATA *mob, CHAR_DATA *ch )
{
    if ( IS_NPC( mob )
    &&   HAS_PROG( mob->pIndexData, LOOK_PROG ) )
	mprog_percent_check( mob, ch, NULL, NULL, LOOK_PROG );
...
{
    bool executed = FALSE;
	if(!IS_NPC(ch)) call_lua_num (ch, "use", obj->pIndexData->vnum);
...
{
	if(!IS_NPC(ch)) call_lua_num (ch, "repair", obj->pIndexData->vnum);
...
{
	if(!IS_NPC(ch)) call_lua_num (ch, "drop", obj->pIndexData->vnum);
