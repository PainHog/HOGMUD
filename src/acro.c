...
#define ACRO_SUBMISSIONS_FOR_VOTING  2
...
// #define ACRO_PRETTY_PRINT_ENTRY
...
void acro_check(void)
{
	...
	else if (!tie)
	{
		CHAR_DATA	*ch;
	    maxap->wins++;
	    announce_acro("%s wins the round with %d votes and gains %d glory!\n\r", maxap->name, maxap->votes, maxap->votes + maxap->wins);
		/* add glory to winner - votes+wins */
		ch = get_char_for_ap(maxap);
		ch->pcdata->quest_accum += maxap->votes + maxap->wins;
		ch->pcdata->quest_curr  += maxap->votes + maxap->wins;
	}
	else
	{
