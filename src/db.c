...
sh_int			gsn_riposte;
sh_int			gsn_shadow_venom;
sh_int			gsn_snipe;

/* thief & warrior */
sh_int          	gsn_disarm;
sh_int			gsn_enhanced_damage;
sh_int			gsn_enhanced_hit;
sh_int			gsn_kick;
sh_int			gsn_parry;
sh_int			gsn_engage;
sh_int			gsn_rescue;
...
sh_int			gsn_tumble;
sh_int			gsn_overrun;
sh_int			gsn_foot_stomp;
sh_int			gsn_enhanced_dual_wield;
...
sh_int			gsn_magika_armor;
sh_int			gsn_mute;
sh_int			gsn_salt_wounds;
/*
 * Locals.
 */
...
void    load_scoremobm     args( ( void ) );
void    load_scorepk	args( ( void ) );
void    load_scoremk    args( ( void ) );
void	load_scoreqp    args( ( void ) );
void    load_scoregold  args( ( void ) );
void	load_scoremp	args( ( void ) );
void	load_scoresp	args( ( void ) );
void	load_scorexp	args( ( void ) );
...
void boot_db( bool fCopyOver )
{
	...
	for ( x = 0; x < MAX_LAYERS; x++ )
    {
        ...
        save_mob_equipment[wear][x] = NULL;
    }
	...
	*/
	...
	ASSIGN_GSN( gsn_chill_touch,	"chill touch" );
        ASSIGN_GSN( gsn_ice_blast,      "ice blast" );
	...
	ASSIGN_GSN( gsn_overrun,	"overrun" );
	ASSIGN_GSN( gsn_magika_armor, "magika armor" );
	ASSIGN_GSN( gsn_mute,		"mute" );
	ASSIGN_GSN( gsn_riposte,	"riposte" );
	//~ ASSIGN_GSN( gsn_salt_wounds,	"salt wounds" );
	ASSIGN_GSN( gsn_crush,		"crush" );
	ASSIGN_GSN( gsn_shadow_venom, "shadow venom" );
	ASSIGN_GSN( gsn_snipe,		"snipe" );
	//~ ASSIGN_GSN( gsn_foot_stomp,	"foot stomp" );
	ASSIGN_GSN( gsn_enhanced_dual_wield, "enhanced dual wield" );

	// crafting skills and stuff follow
	ASSIGN_GSN( gsn_gather,		"gather" 	);
	ASSIGN_GSN( gsn_ignite,		"ignite" 	);
	ASSIGN_GSN( gsn_skin,		"skin" 		);
	ASSIGN_GSN( gsn_mine,		"mine" 		);
	ASSIGN_GSN( gsn_tan,		"tan" 		);
	ASSIGN_GSN( gsn_smelt,		"smelt" 	);
	ASSIGN_GSN( gsn_cut, 		"cut"		);
	ASSIGN_GSN( gsn_cook, 		"cook"		);
	ASSIGN_GSN( gsn_butcher, 	"butcher"	);
	ASSIGN_GSN( gsn_craft, 		"craft"		);
	ASSIGN_GSN( gsn_grind, 		"grind"		);
	ASSIGN_GSN( gsn_forge,		"forge"		);
	ASSIGN_GSN( gsn_polish,		"polish"	);
	ASSIGN_GSN( gsn_tailor,		"tailor"	);
    }
...
*/
    ...
    fread_questmaster();
	log_string("Awakening Questmaster");
    init_area_weather();
        log_string("Loading Scoreboards");
    load_scorepk();
    load_scoregold();
    load_scoremobm();
    load_scoreqp();
...
void load_economy( AREA_DATA *tarea, FILE *fp )
{
	...
	tarea->low_economy = load_new_economy(tarea);
    return;
}
...
void load_mobiles( AREA_DATA *tarea, FILE *fp )
{
	...
	if ( letter == 'C' ) /* Realms complex mob 	-Thoric */
	{
	    ...
	    pMobIndex->perm_lck			= fread_number( fp );
 	    pMobIndex->saving_damage	= fread_number( fp );
...
void load_objects( AREA_DATA *tarea, FILE *fp )
{
	...
	sscanf( ln, "%d %d %d %d %d %d %d %d %d %d %d %d %d %d %d",
		 &x1,  &x2,  &x3,  &x4,  &x5,
         &x6,  &x7,  &x9,  &x9, &x10,
        &x11, &x12, &x13, &x14, &x15 );
...
void load_rooms( AREA_DATA *tarea, FILE *fp )
{
	...
	/* load up random job data stuffs */
	...
	//~ case SECT_FOREST:
		//~ for(i = 0; i < 10; i++)
			//~ pRoomIndex->job_items[i] = number_range(1,100);
		//~ break;
	//~ default:
		//~ break;
	//~ }
...
void area_update( void )
{
	...
	if ( pArea->nplayer > 0 && pArea->age == (reset_age-1) )
	{
	    ...
	    /* Rennard */
	    if ( pArea->resetmsg )
            call_va(L_mud, "sprintf", "ss>s", "%s\n\r", pArea->resetmsg, &buf);
		//~ sprintf( buf, "%s\n\r", pArea->resetmsg );
	    else
            call_va(L_mud, "sprintf", "s>s", "%s", "You hear some sqeaking sounds...\n\r", &buf);
	 ...
	 */
    /*
     * Changed so that areas will not reset while players are in them, with the
     * exception of the mud school area, which will always reset whether there
     * are players in it or not. -Darwin 2008-01-31
     */
    ROOM_INDEX_DATA *pRoomIndex;
    pRoomIndex = get_room_index( ROOM_VNUM_SCHOOL );
	if ( (pArea->nplayer == 0 && pArea->age >= reset_age) || pArea == pRoomIndex->area )
	{
...
*/
    ...
    mob->armor		= 100 - (mob->level * 8);

    if ( xIS_SET( mob->affected_by, AFF_MIGHTY))
    {
      mob->max_hit		= ( ( (mob->level * 15) + (mob->level * mob->level) ) * 2);
    }
    else
...
//				      + pMobIndex->hitplus;
    ...
    if (mob->level <= 30)
        mob->gold			= ( mob->level + number_range( 5, 25) );
    else if (mob->level > 30 && mob->level <= 50)
	mob->gold			= ( mob->level + number_range(50, 150) );
    else if (mob->level > 50)
	mob->gold			= ( (mob->level * 2) + number_range(100, 250) );
    ...
    else if (mob->level > 10 && mob->level <= 20)
    {
       mob->barenumdie          = 5;
       mob->baresizedie         = 5;
    }
    else if (mob->level > 20 && mob->level <= 30)
    {
	mob->barenumdie		= 6;
	mob->baresizedie	= 6;
    }
    else if (mob->level > 30 && mob->level <= 40)
    { 
        mob->barenumdie         = 7;
        mob->baresizedie        = 7; 
    }
    else if (mob->level > 40 && mob->level <= 50)
    { 
        mob->barenumdie         = 8;
        mob->baresizedie        = 8; 
    }
    ...
    mob->damplus                = pMobIndex->level;
		
    mob->perm_str		= pMobIndex->perm_str + (pMobIndex->level / 5);
    mob->perm_dex		= pMobIndex->perm_dex + (pMobIndex->level / 5);
    mob->perm_wis		= pMobIndex->perm_wis + (pMobIndex->level / 5);
    mob->perm_int		= pMobIndex->perm_int + (pMobIndex->level / 5);
    mob->perm_con		= pMobIndex->perm_con + (pMobIndex->level / 5);
    mob->perm_cha		= pMobIndex->perm_cha + (pMobIndex->level / 5);
    mob->perm_lck 		= pMobIndex->perm_lck + (pMobIndex->level / 5);
    mob->hitroll		= pMobIndex->level;
    if ( xIS_SET( mob->affected_by, AFF_MIGHTY))
    {
        mob->damroll		= ((pMobIndex->damroll) + (pMobIndex->level / 2));
    }
    else
    {
    ...
    mob->dualclass		= pMobIndex->class;
    mob->xflags			= pMobIndex->xflags;
    mob->saving_damage	= pMobIndex->saving_damage;
...
void make_wizlist( )
{
  ...
  unlink( WIZLIST_FILE );
  sprintf(buf, " Immortals of the %s!", sysdata.mud_name);
  towizfile(buf);
/*  towizfile( " Masters of the Realms of Despair!" );*/
  ...
  for ( wiz = first_wiz; wiz; wiz = wiz->next )
  {
	...
	buf[0] = '\0';
      ...
      switch(ilevel)
      {
	case MAX_LEVEL -  0: towizfile( " Implementor" );	break;
	...
	case MAX_LEVEL - 13: towizfile( " Emissary" );		break;
...
void fread_sysdata( SYSTEM_DATA *sys, FILE *fp )
{
	...
	case 'P':
		...
		KEY( "PlayerIndex", sys->PlayerIndex,	fread_number( fp ) );
	...
	if(!sys->PlayerIndex) sys->PlayerIndex = 1;
}
