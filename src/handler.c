...
bool can_see_gs( CHAR_DATA *ch, CHAR_DATA *victim);
...
int get_exp_worth( CHAR_DATA *ch )
{
    ...
    exp += ch->max_hit;
    exp -= (ch->armor-50) * 2;
    exp += ( ch->barenumdie * ch->baresizedie + GET_DAMROLL(ch) ) * 4;
    exp += GET_HITROLL(ch) * ch->level * 5;
...
sh_int get_exp_base( CHAR_DATA *ch )
{
    ...
    if ( IS_NPC(ch) )
      base = 50;
    if (class_table[ch->class] == class_table[ch->dualclass])
    {
	if ( class_table[ch->class] != NULL )
        {
	   base = class_table[ch->class]->exp_base;
	}
	else
	{
	   base = 75;
    ...
    if ( (class_table[ch->class] != class_table[ch->dualclass]) && ( (class_table[ch->class]->exp_base) == (class_table[ch->dualclass]->exp_base) ) )
      base = ( (class_table[ch->class]->exp_base) + ( (class_table[ch->dualclass]->exp_base) * .7) );
    if ( (class_table[ch->class] != class_table[ch->dualclass]) && ( (class_table[ch->class]->exp_base) > (class_table[ch->dualclass]->exp_base) ) )
      base = ( (class_table[ch->class]->exp_base) + ( (class_table[ch->dualclass]->exp_base) * .7) );
    if ( (class_table[ch->class] != class_table[ch->dualclass]) && ( (class_table[ch->class]->exp_base) < (class_table[ch->dualclass]->exp_base) ) )
      base = ( (class_table[ch->dualclass]->exp_base) + ( (class_table[ch->class]->exp_base) * .7) );
	if(IS_TIER(ch))
		base = class_table[ch->class]->exp_base;
    return base;
}
...
//   return ( lvl * lvl * lvl * get_exp_base(ch) ); /* Replaced with below to reduce exp tnl -Darwin */
   return ( lvl * lvl * get_exp_base(ch) * (lvl  * 0.95));
...
sh_int get_trust( CHAR_DATA *ch )
{
    ...
    if ( ch->level >= LEVEL_MASTER && IS_RETIRED( ch ) )
      return LEVEL_MASTER;

	if ( !IS_NPC(ch) && (ch->pcdata->tier > 1) && (ch->level < LEVEL_IMMORTAL) )
		return LEVEL_AVATAR;
...
sh_int get_curr_str( CHAR_DATA *ch )
{
	...
	max = 25;
    	...
    	max = 24;
    else if ( class_table[ch->class]->attr_tertiary == APPLY_STR )
    	max = 22;
    else
	max = 20;

	if(!IS_NPC(ch))
	if((18 + race_table[ch->race]->str_plus) > max)
		max = (18 + race_table[ch->race]->str_plus);
...
sh_int get_curr_int( CHAR_DATA *ch )
{
	...
	max = 25;
    	...
    	max = 24;
    else if ( class_table[ch->class]->attr_tertiary == APPLY_INT )
    	max = 22;
    else
	max = 20;

	if(!IS_NPC(ch))
	if((18 + race_table[ch->race]->int_plus) > max)
		max = (18 + race_table[ch->race]->int_plus);
...
sh_int get_curr_wis( CHAR_DATA *ch )
{
	...
	max = 25;
    	...
    	max = 24;
    else if ( class_table[ch->class]->attr_tertiary == APPLY_WIS )
    	max = 22;
    else
	max = 20;

	if(!IS_NPC(ch))
	if((18 + race_table[ch->race]->wis_plus) > max)
		max = (18 + race_table[ch->race]->wis_plus);
...
sh_int get_curr_dex( CHAR_DATA *ch )
{
	...
	max = 25;
    	...
    	max = 24;
    else if ( class_table[ch->class]->attr_tertiary == APPLY_DEX )
    	max = 22;
    else
	max = 20;

	if(!IS_NPC(ch))
	if((18 + race_table[ch->race]->dex_plus) > max)
		max = (18 + race_table[ch->race]->dex_plus);
...
sh_int get_curr_con( CHAR_DATA *ch )
{
	...
	max = 25;
    	...
    	max = 24;
    else if ( class_table[ch->class]->attr_tertiary == APPLY_CON )
    	max = 22;
    else
	max = 20;

	if(!IS_NPC(ch))
	if((18 + race_table[ch->race]->con_plus) > max)
		max = (18 + race_table[ch->race]->con_plus);
...
sh_int get_curr_cha( CHAR_DATA *ch )
{
	...
	max = 25;
    	...
    	max = 24;
    else if ( class_table[ch->class]->attr_tertiary == APPLY_CHA )
    	max = 22;
    else
	max = 20;

	if(!IS_NPC(ch))
	if((18 + race_table[ch->race]->cha_plus) > max)
		max = (18 + race_table[ch->race]->cha_plus);
...
sh_int get_curr_lck( CHAR_DATA *ch )
{
	...
	max = 25;
    	...
    	max = 24;
    else if ( class_table[ch->class]->attr_tertiary == APPLY_LCK )
    	max = 22;
    else
	max = 20;

	if(!IS_NPC(ch))
	if((18 + race_table[ch->race]->lck_plus) > max)
		max = (18 + race_table[ch->race]->lck_plus);
...
int can_carry_n( CHAR_DATA *ch )
{
	...
	return (int)(URANGE(5, (ch->level) + get_curr_dex(ch), 50) / 2);

    if ( IS_NPC(ch) && xIS_SET(ch->act, ACT_IMMORTAL) )
	return ch->level*200;

    return URANGE(5, (ch->level) + get_curr_dex(ch), 50);
}
...
int can_carry_w( CHAR_DATA *ch )
{
    ...
    if ( IS_NPC(ch) && xIS_SET(ch->act, ACT_PET) )
	return (int)(str_app[get_curr_str(ch)].carry / 2);
...
void modify_skill( CHAR_DATA *ch, int sn, int mod, bool fAdd )
{
	...
	if ( fAdd )
	    ...
	    ch->pcdata->learned[sn] = URANGE(0, ch->pcdata->learned[sn] + mod, 200);
...
void affect_modify( CHAR_DATA *ch, AFFECT_DATA *paf, bool fAdd )
{
    ...
    case APPLY_SPIRITSHIELD:  ch->spiritshield		+= mod; break;
    case APPLY_SPELLDAM:      ch->spelldam	        += mod; break;
    case APPLY_AC:            ch->armor			+= mod;	break;
    case APPLY_HITROLL:       ch->hitroll		+= mod;	break;
    case APPLY_DAMROLL:       ch->damroll		+= mod;	break;
    case APPLY_SAVING_DAMAGE: ch->saving_damage  	+= mod;	break;
	...
	if ( !IS_NPC(ch) )
		...
		URANGE( 0, ch->pcdata->condition[COND_FULL] + mod, 150 );
	break;

    case APPLY_THIRST:
	if ( !IS_NPC(ch) )
	    ch->pcdata->condition[COND_THIRST] = 
		URANGE( 0, ch->pcdata->condition[COND_THIRST] + mod, 150 );
	break;
...
/* spell cast upon wear/removal of an object	-Thoric */
    ...
    case APPLY_CAMO:	modify_skill(ch, gsn_camo,	mod, fAdd); 	break;
    ...
    case APPLY_AMBUSH:  modify_skill(ch, gsn_ambush,	mod, fAdd); 	break;
 ...
 */
void affect_to_char( CHAR_DATA *caster, CHAR_DATA *ch, AFFECT_DATA *paf )
{
    AFFECT_DATA *paf_new;
...
OBJ_DATA *obj_to_char( OBJ_DATA *obj, CHAR_DATA *ch )
{
	...
	if(!IS_NPC(ch)) call_lua_num (ch, "got_object", obj->pIndexData->vnum);
...
int apply_ac( OBJ_DATA *obj, int iWear )
{
    ...
    case WEAR_WWP_1:    return     obj->value[0];
    case WEAR_WWP_2:    return     obj->value[0];
    case WEAR_AURA:     return     obj->value[0];
    }
...
void equip_char( CHAR_DATA *ch, OBJ_DATA *obj, int iWear )
{
    ...
    {
//	bug( "Equip_char: already equipped (%s) (%d).", ch->name, iWear ); // commented out -
	return;                              // bug caused by randoms popping on mobs -Darwin
    }
    ...
    if ( ( IS_OBJ_STAT(obj, ITEM_ANTI_EVIL) && (IS_EVIL(ch) && ch->level < 65)    ) 
            ...
            ||   ( ( obj->item_type == ITEM_WEAPON || obj->item_type == ITEM_ARTWEAPON )
			 ...
			 obj->value[3] != 22 && obj->value[3] != 23 )
                    && ( !IS_NPC(ch) && !IS_IMMORTAL(ch) && (IS_MONK(ch)) ) )
            ||   ( ( obj->item_type == ITEM_WEAPON || obj->item_type == ITEM_ARTWEAPON )
                    && ( !xIS_SET(obj->extra_flags, ITEM_2HAND) && IS_SET(obj->wear_flags, ITEM_WIELD) ) 
                    && ( !IS_NPC(ch) && !IS_IMMORTAL(ch) && (IS_THUG(ch)) ) )
            ||   ( ( obj->item_type == ITEM_WEAPON || obj->item_type == ITEM_ARTWEAPON )
	    ...
	    ||   ( IS_OBJ_STAT(obj, ITEM_CONJURER) && (!IS_CONJURER(ch) && !IS_ARCHMAGI(ch)) )
            ...
            ||   ( IS_OBJ_STAT(obj, ITEM_POISONED)      && ( !IS_NPC(ch) && !IS_IMMORTAL(ch) && !IS_CROOK(ch) )) )
    {
	/*
	...
	if ( loading_char != ch )
	{
	   act( AT_MAGIC, "You are zapped by $p as it falls from your body.", ch, obj, NULL, TO_CHAR );
	   act( AT_MAGIC, "$n is zapped by $p as it falls from $s body.",  ch, obj, ch, TO_ROOM );
	}
	if ( obj->carried_by )
	   obj_from_char( obj );
	obj_to_char( obj, ch );
    ...
    ch->armorworn	-= apply_ac(obj, iWear);
    obj->wear_loc	 = iWear;
...
void obj_from_room( OBJ_DATA *obj )
{
    ...
    {
	bug( "obj_from_room: NULL. # %d", obj->pIndexData->vnum );
	return;
    }
	...
	if( (obj->pIndexData->vnum != OBJ_VNUM_CORPSE_PC)	&& 
		...
		!IS_OBJ_STAT(obj, ITEM_CONJURER)		)
		obj->timer		  = -1; // remove ground rot timer -Darwin
...
OBJ_DATA *obj_to_room( OBJ_DATA *obj, ROOM_INDEX_DATA *pRoomIndex )
{
	...
	if((obj->pIndexData->vnum != OBJ_VNUM_CORPSE_PC)	&& 
	   ...
	   (obj->timer <= 0)								&&
	   !IS_OBJ_STAT( obj, ITEM_CONJURER )					)
		obj->timer		  = 30; // add ground rot timer -Darwin
    if ( item_type == ITEM_FIRE )
	pRoomIndex->light += count;
    falling++;
    obj_fall( obj, FALSE );
...
/*
 * True if char can see victim for WHO
 */
bool can_see_who( CHAR_DATA *ch, CHAR_DATA *victim )
{
	...
	if( !IS_NPC(victim) && !IS_NPC(ch) )
	{
		for(i = 0; i < 10; i++)
		{
			if(ch->gs_by[i])
			{
				if(!strcmp(ch->gs_by[i], victim->name))
					return TRUE;
			}
			else if(victim->gs_by[i])
			{
				if(!strcmp(victim->gs_by[i], ch->name))
					return TRUE;
			}
		}
	}
    ...
    if ( !IS_NPC(victim) && !IS_NPC(ch) && xIS_SET(victim->act, PLR_WIZINVIS)
...
bool can_see( CHAR_DATA *ch, CHAR_DATA *victim )
{
	...
	return TRUE;
/*
	if ( !IS_NPC(victim) && !IS_NPC(ch) && ch->pcdata->clan && victim->pcdata->clan &&
         xIS_SET(ch->act, PLR_GODSIGHT) && !strcmp(ch->pcdata->clan->deity, victim->pcdata->clan->deity) )
	return TRUE;
*/
    ...
    &&   get_trust( ch ) < victim->mobinvis && !IS_IMMORTAL(ch) )
        return FALSE;
...
*/
	...
	&&  (!IS_AFFECTED(ch, AFF_DETECT_INVIS) && !IS_AFFECTED(ch, AFF_TRUESIGHT)) )
	  return FALSE;

	if ( IS_AFFECTED(victim, AFF_HIDE) && ( !IS_AFFECTED(ch, AFF_DETECT_HIDDEN) && !IS_AFFECTED(ch, AFF_DEMONSIGHT)
...
*/
    ...
    if ( IS_AFFECTED(victim, AFF_CAMO) && !IS_AFFECTED(ch, AFF_DEMONSIGHT) ) 
        return FALSE;
...
bool can_see_obj( CHAR_DATA *ch, OBJ_DATA *obj )
{
    ...
    if ( (obj->item_type == ITEM_LIGHT || obj->item_type == ITEM_ARTLIGHT) && obj->value[2] != 0 )
	return TRUE;
...
char *affect_loc_name( int location )
{
    ...
    case APPLY_SPIRITSHIELD:    return "spiritshield";
    case APPLY_SPELLDAM:	return "spell damage";
    ...
    case APPLY_SAVING_DAMAGE:	return "save vs damage";
    ...
    case APPLY_EXT_AFFECT:	return "affected_by";
    ...
    case APPLY_AMBUSH:		return "ambush";
...
char *affect_bit_name( EXT_BV *vector )
{
    ...
    if ( xIS_SET(*vector, AFF_DETECT_GOOD  )) strcat( buf, " detect_good"   );
    ...
    if ( xIS_SET(*vector, AFF_IMP_INVIS    )) strcat( buf, " imp_invis"     );
    ...
    if ( xIS_SET(*vector, AFF_CAMO	   )) strcat( buf, " camo"	    );
    ...
    if ( xIS_SET(*vector, AFF_CORRUPT      )) strcat( buf, " corrupt"       );
    if ( xIS_SET(*vector, AFF_BALANCED	   )) strcat( buf, " balanced"	    );
    if ( xIS_SET(*vector, AFF_MIGHTY	   )) strcat( buf, " mighty"	    );
    if ( xIS_SET(*vector, AFF_DEMONSIGHT   )) strcat( buf, " demonsight"    );
    if ( xIS_SET(*vector, AFF_BOUND	   )) strcat( buf, " bound"	    );
    if ( xIS_SET(*vector, AFF_EMPOWERED    )) strcat( buf, " empowered"     );
    if ( xIS_SET(*vector, AFF_TONGUES	   )) strcat( buf, " tongues"	    );
    if ( xIS_SET(*vector, AFF_AURALINK     )) strcat( buf, " auralink"      );
	if ( xIS_SET(*vector, AFF_MUTE		   )) strcat( buf, " mute"			);
...
char *extra_bit_name( EXT_BV *extra_flags )
{
    ...
    if ( xIS_SET(*extra_flags, ITEM_GOOD)       ) strcat( buf, " good"	       );
    if ( xIS_SET(*extra_flags, ITEM_INVIS)	) strcat( buf, " invis"        );
    if ( xIS_SET(*extra_flags, ITEM_MAGIC)	) strcat( buf, " magic"        );
    if ( xIS_SET(*extra_flags, ITEM_ENCHANTED)  ) strcat( buf, " enchanted"    );
    if ( xIS_SET(*extra_flags, ITEM_NODROP)	) strcat( buf, " nodrop"       );
    if ( xIS_SET(*extra_flags, ITEM_NOBURN)	) strcat( buf, " noburn"       );
    ...
    if ( xIS_SET(*extra_flags, ITEM_GOLEM)	) strcat( buf, " golem"   );
    if ( xIS_SET(*extra_flags, ITEM_CONJURER)   ) strcat( buf, " conjurer" );
    ...
    if ( xIS_SET(*extra_flags, ITEM_2HAND)	) strcat( buf, " 2handed"      );
	if ( xIS_SET(*extra_flags, ITEM_NOJUNK) ) strcat( buf, " no_junk"      );
	if ( xIS_SET(*extra_flags, ITEM_RANDOM) ) strcat( buf, " random"      );
    return ( buf[0] != '\0' ) ? buf+1 : "none";
}
 ...
 *  Commented out by Drazuk for Samson's stat roller code
 */
...
/* 	Racial bonus calculations moved to this funciton and removed form comm.c - Samson 2-2-98 */
...
void showaffect( CHAR_DATA *ch, AFFECT_DATA *paf )
{
	...
	if ( paf->location != APPLY_NONE && paf->modifier != 0 )
	{
	    ...
	    {
	      default:
		sprintf( buf, "&GAffects &W%s&D by &W%d&D.\n\r",
		  affect_loc_name( paf->location ), paf->modifier );
		break;
              case APPLY_EXT_AFFECT:
                sprintf( buf, "&GAffects &W%s&D by&W",
		...
		break;
	      ...
	      case APPLY_WEARSPELL:
	      case APPLY_REMOVESPELL:
		sprintf( buf, "&GCasts spell &c'&R%s&c'&D\n\r",
...
/* Function to check if a mob is a shopkeeper -Darwin */
bool iskeeper( CHAR_DATA *ch)
{
	if(ch && IS_NPC(ch) && ch->pIndexData->iskeeper)
        return ch->pIndexData->iskeeper;
...
void economize_mobgold( CHAR_DATA *mob )
{
	...
	{
		mob->gold = 0;
		return; // Don't want shoppies to carry ANY gold -Darwin
	}
