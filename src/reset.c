		 ...
		 ROOM_INDEX_DATA *aRoom )
{
  ...
  {
    if (call_mud_lua ("reset_area", pArea->filename))
        return;
...
void do_rreset( CHAR_DATA *ch, char *argument )
{
  ...
  if ( ch->substate == SUB_REPEATCMD )
  {
    ...
    if ( !pRoom )
    {
      ...
      bug("do_rreset: %s's dest_buf points to invalid room", (long int)ch->name);
    }
    ch->substate = SUB_NONE;
    ch->dest_buf = NULL;
    return;
...
int generate_itemlevel( AREA_DATA *pArea, OBJ_INDEX_DATA *pObjIndex )
{
	...
	//~ {
	    ...
	    //~ case ITEM_ARTARMOR:  olevel = number_range( min+4, max+1 ); break;
	    //~ case ITEM_WEAPON:	 olevel = number_range( min+4, max+1 );	break;
	    //~ case ITEM_ARTWEAPON: olevel = number_range( min+4, max+1 ); break;
	//~ }
    //~ return olevel;
    return 0;
}
...
void reset_area( AREA_DATA *pArea )
{
  ...
  if ( !pArea->first_reset )
  {
    bug( "%s: reset_area: no resets", (long int)pArea->filename );
    return;
  }
  level = 0;
  for ( pReset = pArea->first_reset; pReset; pReset = pReset->next )
  {
	...
	if ( pRoomPrev && IS_SET(pRoomPrev->room_flags, ROOM_PET_SHOP) )
      ...
      {
	int artchance;
        artchance = UMAX( 2, (mob->level * 2) ); // changed from / 6

        if (number_percent() <= 25) // changed from 6
        {
     ...
     int relicchance;
     relicchance = UMAX( 1, (mob->level / 16) ); // changed from 16

     if (number_percent() <= relicchance)
     {
        int relicpop;
	relicpop = number_range(1,100); // *shrug* Not sure why this is here... not used anywhere -Darwin

// heh, i must of had "relicchance" below, making "relicpop" useless...but it needs to be in there
// because "relicchance" can never be >= 90, so relics will never pop that way...
// went ahead and changed relicpop back to its 98...use "mwhere relic" to see how many actually spawn
// ----Drazuk

	if (relicpop >= 98) // increase this a bit.. changed from 98
	{
	...
	bug ( buf, 0 );
      ...
      //~ if ( pObjIndex->count >= pReset->arg2 && pObjIndex->rent >= MIN_RENT )
      //~ {
	  //~ obj = NULL;
	  //~ lastobj = NULL;
	  //~ break;
	...
	xSET_BIT(obj->extra_flags, ITEM_INVENTORY);
      ...
      else
	  {
		  ...
		  obj->cost = 0; // Testing to see if this will cause stock items to have 0 value -Darwin
	  }
