...
/* grub_truncate a char string if it's length exceeds a given value. */
void grub_trunc(char *s, int len)
{
   if ( strlen(s) > len )
      s[len] = '\0';
}
...
void do_owhere( CHAR_DATA *ch, char *argument )
{
    ...
    int icnt=0, vnum=0;
    char heading[] =
"    Vnum   ID    Short Desc        Vnum  Room/Char          Vnum  Container\n\r";
    ...
    for ( obj = first_object; obj; obj = obj->next )
    {
        ...
        sprintf(field, "%-20s", obj_short(obj));
        grub_trunc(field, 20);
        sprintf(buf, "%3d %5d %6d %-18s ", ++icnt, obj->pIndexData->vnum, obj->obj_id, field);
...
switch (loc)
  {
     ...
     case APPLY_SAVING_DAMAGE : {r->n[OSAV0]+= mod; break;}
...
#else
  if ( ( fp = fopen( "../grub/grub.new", "r" ) ) == NULL )
#endif
     return;
