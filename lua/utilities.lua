...
function heading (title, colour)
  local len = string.gsub(title, "&%a", "")
  local hyphens = math.floor ((78 - (#len + #len%2)) / 2)
  send (colour or "&Y", ("-"):rep (hyphens), " ", title, " ", ("-"):rep (hyphens+#len%2))
end -- heading

function heading2 (title, colour)
  local len = string.gsub(title, "&%a", "")
  local hyphens = math.floor ((78 - (#len + #len%2)) / 2)
  send (colour or "&Y", ".*", ("~"):rep (hyphens-2), " ", title, " ", ("~"):rep ((hyphens+#len%2)-2), "*.")
end -- heading2
