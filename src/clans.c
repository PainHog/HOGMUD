...
void do_roster( CHAR_DATA *ch, char *argument )
{
   ...
   {
      send_to_char( "Usage: roster 'following name'\n\r", ch );
      send_to_char( "Usage: roster 'following name' remove <name>\n\r", ch );
      return;
   }
   ...
   if( !argument || argument[0] == '\0' )
   {
      ch_printf( ch, "Membership roster for the %s %s\n\r\n\r", clan->name,
         clan->clan_type == CLAN_ORDER ? "Guild" : "Following" );
      ch_printf( ch, "%-15.15s  %-15.15s %-6.6s %-6.6s %-6.6s %s\n\r", "Name", "Class", "Level", "Kills", "Deaths", "Joined on" );
      send_to_char( "-------------------------------------------------------------------------------------\n\r", ch );
      for( roster = clan->first_member; roster; roster = roster->next )
      {
      ...
      ch_printf( ch, "\n\rThere are %d member%s in %s\n\r", total, total == 1 ? "" : "s", clan->name );
      return;
   }
   ...
   if( !str_cmp( arg2, "remove" ) )
   {
      if ( !IS_IMMORTAL(ch) )
      {
	send_to_char( "Only Immortals can remove a name from the rosters.\n\r", ch );
	return;
      }
...
/* Deposit gold into following account for following wealth
code. First draft, February 1, 2004 ~Syriac */
void do_fdeposit( CHAR_DATA *ch, char *argument )
{
    ...
    if ( IS_IMMORTAL(ch))
    {
	pager_printf_color(ch, "&CImmortals may not deposit in following acounts.\n\r");
	return;
    }

	clan = ch->pcdata->clan;

	if(!clan) // check for clan -Darwin
	{
		send_to_char("&WYou do not have a clan with which to deposit gold.\n\r", ch);
		return;
	}

    if ( argument[0] == '\0' )
    {
    	pager_printf_color( ch, "&WSyntax: fdeposit <amount>\n\r");
    	return;
    }
  ...
  argument = one_argument( argument, arg1 );
    ...
    if ( arg1 == '\0' )
    {
	 pager_printf_color(ch, "&wHow much &Ygold&w would you like to fdeposit?\n\r");
   	 return;
    }
    ...
    if ( amount > ch->gold )
    {
    	pager_printf_color(ch, "&WYou do not have that much gold.\n\r");
   	return;
    }
    
    if ( amount <= 0 )
    {
    	pager_printf_color(ch, "You cannot fdeposit negative amounts.\n\r");
    	return;
    }
  ...
  ch_printf( ch, "You fdeposit %d gold.\n\r", amount );
  save_char_obj( ch );
  save_clan( clan );
  return;
}

void do_fwithdraw( CHAR_DATA *ch, char *argument )
{
    unsigned long amount;
    char arg1[MAX_INPUT_LENGTH];
    CLAN_DATA *clan;
    ...
    if ( !IS_IMMORTAL(ch))
    {
        pager_printf_color(ch, "&CMortals may not make withdrawls from following acounts.\n\r");
        return;
    }
    ...
    if ( ch->pcdata->clan->deity != ch->name )
    {
	pager_printf_color(ch, "You may not withdraw any gold from your following's account.\n\r");
	return;
    }
    ...
    if ( argument[0] == '\0' )
    {
        pager_printf_color( ch, "&WSyntax: fwithdraw <amount>\n\r");
        return;
    }
  ...
  argument = one_argument( argument, arg1 );
    ...
    if ( arg1 == '\0' )
    {
         pager_printf_color(ch, "&wHow much &Ygold&w would you like to fwithdraw?\n\r");
         return;
    }
    ...
    {
        pager_printf_color(ch, "&WYou do not have that much gold to withdraw.\n\r");
        return;
    }

    if ( amount <= 0 )
    {
        pager_printf_color(ch, "You cannot fwithdraw negative amounts.\n\r");
        return;
    }
  ...
  clan->balance         -= amount;
  set_char_color( AT_PLAIN, ch );
  ch_printf( ch, "You fwithdraw %d gold.\n\r", amount );
  save_char_obj( ch );
  save_clan( clan );
  return;
}
...
void do_induct( CHAR_DATA *ch, char *argument )
{
	...
	return;
      ...
      {
	if ( victim->pcdata->clan == clan )
	  send_to_char( "This player already belongs to your following!\n\r", ch );
	else
	  send_to_char( "This player already belongs to a following!\n\r", ch );
	return;
      }
    }
...
*/
    ...
    victim->alignment = clan->alignment;
...
void do_setfol( CHAR_DATA *ch, char *argument )
{
    char arg1[MAX_INPUT_LENGTH];
    ...
    if ( arg1[0] == '\0' )
    {
	send_to_char( "Usage: setfol <name> <field> <deity|leader|number1|number2> <player>\n\r", ch );
	send_to_char( "\n\rField being one of:\n\r", ch );
...
void do_showfol( CHAR_DATA *ch, char *argument )
{   
    CLAN_DATA *clan;
    ...
    if ( argument[0] == '\0' )
    {
	send_to_char( "Usage: showfol <following>\n\r", ch );
	return;
    }
...
void do_makefol( CHAR_DATA *ch, char *argument )
{
    char filename[256];
    CLAN_DATA *clan;
    bool found;
    ...
    if ( !argument || argument[0] == '\0' )
    {
	send_to_char( "Usage: makefol <following name>\n\r", ch );
	return;
    }
...
void do_finfo( CHAR_DATA *ch, char *argument )
{
    CLAN_DATA *clan;
    int count = 0;
    ...
    if ( argument[0] == '\0' )
    {
        ...
        send_to_char( "\n\rFollowing                  Deity         Leader           Pkills:    Avatar      Other\n\r_________________________________________________________________________________________\n\r\n\r", ch );
        for ( clan = first_clan; clan; clan = clan->next )
        {
            if ( clan->clan_type == CLAN_ORDER || clan->clan_type == CLAN_GUILD )
              continue;
            set_char_color( AT_GREY, ch);
            ch_printf( ch, "%-26s %-13s %-13s", clan->name, clan->deity, clan->leader );
            set_char_color( AT_LBLUE, ch );
            ch_printf( ch, "                %5d      %5d\n\r", 
		clan->pkills[6],
        ...
        set_char_color( AT_LBLUE, ch );
        if ( !count )
          send_to_char( "There are no Followings currently formed.\n\r", ch );
        else
          send_to_char( "_________________________________________________________________________________________\n\r\n\rUse 'finfo <following>' for detailed information and a breakdown of victories.\n\r", ch );
        return;
    }
...
if (ch->class == CLASS_WARRIOR)
  chance = 70;
if (ch->class == CLASS_CHAMPION)
  chance = 70;
if (ch->class == CLASS_RANGER)
  chance = 60;
if (ch->class == CLASS_DRUID)
  chance = 45;
if (ch->class == CLASS_BISHOP)
  chance = 45;
if (ch->class == CLASS_MONK)
  chance = 45;
if (ch->class == CLASS_THUG)
  chance = 40;
...
if (ch->class == CLASS_THIEF)
  chance = 30;
if (ch->class == CLASS_JESTER)
  chance = 30;
if (ch->class == CLASS_DIABOLIST)
  chance = 20;
if (ch->class == CLASS_ARCHMAGI)
  chance = 20;
