...
"water_noswim", "underwater", "air", "desert", "somewhere", "oceanfloor",
"underground", "lava", "swamp", "boat", "r2", "r3", "r4", "r5", "r6", "r7",
"r8", "r9", "r10", "r11", "r12", "r13", "r14", "r15", "r16"
};

char *	const	r_flags	[] =
{
"dark", "death", "nomob", "indoors", "healing", "mental", "chaotic",
"nomagic", "tunnel", "private", "safe", "solitary", "petshop", "norecall",
"donation", "nodropall", "silence", "logspeech", "nodrop", "folstoreroom",
"nosummon", "noastral", "teleport", "teleshowdesc", "nofloor",
"nosupplicate", "arena", "norun", "locker", "bloodroom", "prototype", "dnd"
};
...
"nolocate", "groundrot", "lootable", "ided", "good", "quest",
"noburn", "2handed", "noquest", "golem", "conjurer", "enchanted", "no_junk",
"random", "pet_eq" };
...
char *	const	w_flags	[] =
{
"take", "head", "eyes", "ears", "face", "neck", "body", "arms", "wrist",
"hands", "finger", "about", "back", "waist", "legs", "ankle", "feet", "wield",
"_dual_", "shield", "hold", "pride","aura","r6",
"r7","r8","r9","r10","r11","r12","r13"
};

char *	const	item_w_flags	[] =
{
"take", "head", "eyes", "ears", "face",  "neck", "neck", "body", "arms",
"wrist", "wrist", "hands", "finger", "finger", "about", "back", "waist",
"legs", "ankle", "ankle", "feet", "wield", "dual", "shield", "hold",
"pride","pride","aura","r7","r8","r9","r10","r11","r12","r13"
};
...
"treasure", "armor", "potion", "worn", "furniture", "trash", "_oldtrap",
...
"salve", "cook", "keyring", "comp", "artweapon", "artarmor", "arttreasure",
"artworn", "artlight", "mix", "chance"
};
...
"gold", "experience", "armor", "hitroll", "damroll", "save_damage", "save_rod",
...
"roomlight", "televnum", "teledelay", "camouflage", "ambush", "spiritshield",
"spelldam", "jump", "wait"
};
...
"detect_hidden", "imp_invis", "sanctuary", "faerie_fire", "infrared", "curse",
"corrupt", "poison", "protect", "_paralysis", "sneak", "hide", "sleep",
"charm", "flying", "pass_door", "floating", "truesight", "detect_traps",
"scrying", "fireshield", "shockshield", "detect_good", "iceshield", "possess", 
"berserk", "aqua_breath", "recurringspell", "empowered", "tongues",
"balanced", "auralink", "unused5", "mighty", "camouflage", "demonsight",
"bound", "mute"
};

char *	const	act_flags [] =
{
"npc", "sentinel", "scavenger", "healer", "banker", "aggressive", "stayarea",
"wimpy", "pet", "innkeeper", "guildmaster", "immortal", "bribable", "polymorphed",
"undertaker", "guardian", "running", "trainer", "mountable", "mounted",
"scholar", "secretive", "noquest", "mobinvis", "noassist", "astralblock",
"pacifist", "noattack", "equipped", "boat", "aquatic", "questmaster",
...
"nstart", "dnd", "idle", "r12", "r13", "r14", "r15", "r16", "r17", "r18",
...
"deny", "freeze", "thief","killer", "bully", "ansi", "rip", "nice", 
"flee" ,"autogold", "automap", "afk", "invisprompt", "questor", "autoglance",
"compass", "consent", "seedam", "stalk", "exempt", "godsight", "mobm", "tier",
"gambler", "mobm_expert", "autoupdate", "autoassist", "abbaffs", "tierconvert",
"squestor", "mquestor", "shortwho"
};
...
{
"light", "head", "eyes", "ears", "face", "neck1", "neck2", "body",
"arms", "wrist1", "wrist2", "hands", "finger1", "finger2", "about", "back",
"waist", "legs", "ankle1", "ankle2", "feet", "wield", "dual",
"shield", "hold", "pride1", "pride2", "aura"
};
...
"gouge", "backstab", "drain", "firebreath", "frostbreath",
"acidbreath", "lightnbreath", "gasbreath", "poison", "unused1", "unused2",
...
"unused1", "unused2", "unused3", "teleport", "unused4", "unused5", "unused6",
"unused7", "disarm", "iceshield", "grip", "truesight", "unused8", "unused9"
};
...
char *	const	gather_name [] =
{ 
	"oak", "pine", "ash", "birch", "hawthorn",
	"yew", "walnut", "willow", "blackthorn", "elder"
};

char *  const   mine_name [] =
{
	"coal", "iron", "amethyst", "gold", "silver",
	"bloodstone", "diamond", "jade", "platinum", "titanium"
};
...
void do_goto( CHAR_DATA *ch, char *argument )
{
   ...
   if ( !is_number(arg) && (fch=get_char_world(ch, arg)) )
   {
    ...
    {
        call_va(L_mud, "parse_bamfout", "ss>s", ch->name, (ch->pcdata && ch->pcdata->bamfout[0] != '\0') ? ch->pcdata->bamfout : "", &buf);
        act( AT_IMMORT, "$T", ch, NULL, buf,  TO_CANSEE );
    }
    ...
    //~ if ( !xIS_SET(ch->act, PLR_WIZINVIS) )
    {
        call_va(L_mud, "parse_bamfin", "ss>s", ch->name, (ch->pcdata && ch->pcdata->bamfin[0] != '\0') ? ch->pcdata->bamfin : "", &buf);
        act( AT_IMMORT, "$T", ch, NULL, buf, TO_CANSEE );
    }
...
void do_mset( CHAR_DATA *ch, char *argument )
{
	...
	send_to_char( "  str int wis dex con cha lck sex class dual\n\r",	ch );
	send_to_char( "  gold hp mana move practice align race height\n\r",	ch );
	send_to_char( "  weight hitroll damroll armor affected level\n\r",	ch );
	send_to_char( "  acworn thirst drunk full blood flags\n\r",		ch );
    ...
    if ( !str_cmp( arg2, "charmies" ) )
    {
	if ( !can_mmodify( ch, victim ) )
	  return;
	if ( value < 0 || value > 10 )
	{
	    ch_printf( ch, "Charmie range is 0 to 10.\n\r" );
	    return;
	}
	victim->pcdata->charmies = value;
	return;
    }	
	...
	if ( !can_mmodify( ch, victim ) )
	  return;
        ...
        if( IS_NPC(victim) )
	{   
	   send_to_char( "NPC's cannot be dual-classed.\n", ch );
	   return;
	}
	...
	if ( value < 0 || value > LEVEL_AVATAR + 25 )
	{
	...
	if ( !can_mmodify( ch, victim ) )
	  return;
	victim->hitroll = URANGE(0, value, 120);
	return;
    }
	...
	if ( !can_mmodify( ch, victim ) )
	  return;
	victim->damroll = URANGE(0, value, 120);
	return;
    }
    if ( !str_cmp( arg2, "spoint" ) )
    {
		if(IS_NPC(victim))
		{
		...
		if ( value < 0 )
		{
			send_to_char( "Value can only be a positive number.\n\r", ch );
			return;
		}
		victim->pcdata->skillpoints = value;
		return;
    }
    if ( !str_cmp( arg2, "mpoint" ) )
    {
		if(IS_NPC(victim))
		{
			send_to_char("Not on mobiles.\n\r", ch);
		...
		victim->pcdata->magicpoints = value;
		return;
    }
    if ( !str_cmp( arg2, "armorworn" ) )
    {
	if ( !can_mmodify( ch, victim ) )
	   return;
	victim->armorworn = value;
	...
	{
	    send_to_char( "Range: 0 - 200.\n\r", ch );
	    return;
	}

	victim->mobmticks = value;
	return;
    }
	...
	{
	    send_to_char( "The current quest point range is 0 to 150000.\n\r", ch );
	    return;
	}
	...
	{
	    send_to_char( "Not on PC's. Use PCRENAME.\n\r", ch );
	    return;
	}
...
*/
	...
	{
	    send_to_char( "Height range is 0 to 120.\n\r", ch );
	    return;
	}
	victim->height = value;
	send_to_char( "Done.\n\r", ch );
	...
	if ( value < 0 || value > 1000 )
	{
	    send_to_char( "Weight range is 0 to 1000.\n\r", ch );
	    return;
	}
	victim->weight = value;
	send_to_char( "Done.\n\r", ch );
...
void do_oset( CHAR_DATA *ch, char *argument )
{
	...
	{
	   ch_printf(ch, "You are not carrying any %s.\n\r", arg1 );
	   return;
	}
    }
    else
    if ( !obj )
    {
		if(arg1[0] == '!')
		{
			if (( obj = get_obj_world( ch, arg1+1 ) ) == NULL )
			{
    ...
    if ( get_trust(ch) < 60 )
    {
	send_to_char("You can only oset the name, short, long and owner right now.\n\r", ch );
	return;
    }

    if (!str_prefix("value", arg2) || !str_prefix("v", arg2))
    {
        int val;
        call_va(L_mud, "parse_num", "s>i", arg2, &val);
        if(!can_omodify(ch, obj))
            return;
        obj->value[val] = value;
        if(IS_OBJ_STAT(obj, ITEM_PROTOTYPE))
            obj->pIndexData->value[val] = value;
        return;
    }
	...
	int bitv;
        ...
        // Moved to top for xaffected support...- Slicer
	...
	if ((loc >= APPLY_AFFECT && loc < APPLY_WEAPONSPELL) || loc == APPLY_EXT_AFFECT)
	{
	    bitv = 0;
	    while (argument[0] != '\0')
	    {
...
void lua_fold_area( AREA_DATA *tarea, char *filename, bool install )
{
    RESET_DATA		*treset;
    ROOM_INDEX_DATA	*room;
    MOB_INDEX_DATA	*pMobIndex;
    ...
    for ( vnum = tarea->low_m_vnum; vnum <= tarea->hi_m_vnum; vnum++ )
    {
        ...
        ||   pMobIndex->xflags	 != 0   ||   pMobIndex->numattacks != 0 )
          ...
          complexmob = FALSE;
        fprintf( fpout, "\t\tmobiles[%d] = { ",	                vnum				                );
        fprintf( fpout, "name = [[%s]],\n",	                    pMobIndex->player_name		    );
        fprintf( fpout,	"\t\t\tshort_description = [[%s]],\n",	    pMobIndex->short_descr		    );
        fprintf( fpout,	"\t\t\tlong_description = [[%s]],\n",	        strip_cr(pMobIndex->long_descr)	);
        fprintf( fpout, "\t\t\tdescription = [[%s]],\n",	            strip_cr(pMobIndex->description));
        fprintf( fpout, "\t\t\tact_flags = [[%s]],\n",		        ext_flag_string(&pMobIndex->act, act_flags));
        fprintf( fpout, "\t\t\taffected_by = [[%s]],\n",              affect_bit_name( &pMobIndex->affected_by ));
        fprintf( fpout, "\t\t\talign = %d,\n",                        pMobIndex->alignment            );
        fprintf( fpout, "\t\t\tcomplex = %d,\n",      				complexmob ? 1 : 0		        );
        fprintf( fpout, "\t\t\tlevel = %d,\n",	                    pMobIndex->level                );
        fprintf( fpout, "\t\t\tmobthac0 = %d,\n",     				pMobIndex->mobthac0             );
        fprintf( fpout, "\t\t\tac = %d,\n",            				pMobIndex->ac			        );
        fprintf( fpout, "\t\t\thitdice = { dice = %d, size = %d, plus = %d },\n",	pMobIndex->hitnodice,
                        pMobIndex->hitsizedice,
                        pMobIndex->hitplus		);
        fprintf( fpout, "\t\t\tdamdice = { dice = %d, size = %d, plus = %d },\n",	pMobIndex->damnodice,
                        pMobIndex->damsizedice,
                        pMobIndex->damplus		);
    ...
    /* Need to convert to new positions correctly on loadup sigh -Shaddai */
        ...
        if ( complexmob )
        {
            fprintf( fpout, "\t\t\tperm_str = %d,\n", pMobIndex->perm_str);
            fprintf( fpout, "\t\t\tperm_int = %d,\n", pMobIndex->perm_int);
            fprintf( fpout, "\t\t\tperm_wis = %d,\n", pMobIndex->perm_wis);
            fprintf( fpout, "\t\t\tperm_dex = %d,\n", pMobIndex->perm_dex);
            fprintf( fpout, "\t\t\tperm_con = %d,\n", pMobIndex->perm_con);
            fprintf( fpout, "\t\t\tperm_cha = %d,\n", pMobIndex->perm_cha);
            fprintf( fpout, "\t\t\tperm_lck = %d,\n", pMobIndex->perm_lck);
            fprintf( fpout, "\t\t\tsaving_damage = %d,\n", pMobIndex->saving_damage);
            fprintf( fpout, "\t\t\tsaving_wand = %d,\n", pMobIndex->saving_wand);
            fprintf( fpout, "\t\t\tsaving_para_petri = %d,\n", pMobIndex->saving_para_petri);
            fprintf( fpout, "\t\t\tsaving_breath = %d,\n", pMobIndex->saving_breath);
            fprintf( fpout, "\t\t\tsaving_spell_staff = %d,\n", pMobIndex->saving_spell_staff);
            fprintf( fpout, "\t\t\trace = %d,\n", pMobIndex->race);
            fprintf( fpout, "\t\t\tclass = %d,\n", pMobIndex->class);
            fprintf( fpout, "\t\t\theight = %d,\n", pMobIndex->height);
            fprintf( fpout, "\t\t\tweight = %d,\n", pMobIndex->weight);
            fprintf( fpout, "\t\t\tspeaks = %d,\n", pMobIndex->speaks);
            fprintf( fpout, "\t\t\tspeaking = [[%s]],\n", lang_names[pMobIndex->speaking-1]);
            fprintf( fpout, "\t\t\tnumattacks = %d,\n", pMobIndex->numattacks);
            fprintf( fpout, "\t\t\thitroll = %d,\n", pMobIndex->hitroll);
            fprintf( fpout, "\t\t\tdamroll = %d,\n", pMobIndex->damroll);
            fprintf( fpout, "\t\t\txflags = [[%s]],\n", flag_string(pMobIndex->xflags, part_flags));
            fprintf( fpout, "\t\t\tresistant = %d,\n", pMobIndex->resistant);
            fprintf( fpout, "\t\t\timmune = %d,\n", pMobIndex->immune);
            fprintf( fpout, "\t\t\tsusceptible = %d,\n", pMobIndex->susceptible);
            fprintf( fpout, "\t\t\tattacks = [[%s]],\n",	print_bitvector(&pMobIndex->attacks) );
            fprintf( fpout, "\t\t\tdefenses = [[%s]]\n",	print_bitvector(&pMobIndex->defenses) );
        }
        if ( pMobIndex->mudprogs )
        {
            fprintf( fpout, "\t\t\tmudprogs = {},\n" );
            x = 1;
            for ( mprog = pMobIndex->mudprogs; mprog; mprog = mprog->next )
            {
                fprintf( fpout, "\t\t\t\tmudprogs[%d] = { type = [[%s]], arglist = [[%s]], comlist = [==[%s]==] },\n",
                    x,
                    mprog_type_to_name( mprog->type ),
                    mprog->arglist, strip_cr(mprog->comlist) );
                x++;
    ...
    for ( vnum = tarea->low_o_vnum; vnum <= tarea->hi_o_vnum; vnum++ )
    {
        ...
        if ( install )
          xREMOVE_BIT( pObjIndex->extra_flags, ITEM_PROTOTYPE );
        fprintf( fpout, "\t\tobjects[%d] = { ",	vnum				);
        fprintf( fpout, "name = [[%s]],\n",	pObjIndex->name			);
        fprintf( fpout, "\t\tshort_description = [[%s]],\n",	pObjIndex->short_descr		);
        fprintf( fpout, "\t\tlong_description = [[%s]],\n",	pObjIndex->description		);
        fprintf( fpout, "\t\taction_description = [==[%s]==],\n",	pObjIndex->action_desc		);
        fprintf( fpout, "\t\ttype = [[%s]],\n\t\textra_flags = [[%s]],\n\t\twear_flags = [[%s]],\n",	
                o_types[pObjIndex->item_type], ext_flag_string(&pObjIndex->extra_flags, o_flags), 
        ...
        fprintf( fpout, "\t\tweight = %d,\n\t\tcost = %d,\n",	pObjIndex->weight, pObjIndex->cost );
            ...
            case ITEM_POTION:
            case ITEM_SCROLL:
                fprintf( fpout, "\t\tpill_potion_scroll_spells = {[[%s]], [[%s]], [[%s]]},\n",
                        IS_VALID_SN(pObjIndex->value[1])?
                skill_table[pObjIndex->value[1]]->name:"NONE",
                IS_VALID_SN(pObjIndex->value[2])?
                skill_table[pObjIndex->value[2]]->name :"NONE",
            ...
            case ITEM_STAFF:
            case ITEM_WAND:
                    fprintf( fpout, "\t\tstaff_wand_spell = [[%s]],\n",
                            IS_VALID_SN(pObjIndex->value[3])?  
                        skill_table[pObjIndex->value[3]]->name:"NONE" );
        
                break;
            case ITEM_SALVE:
                    fprintf( fpout, "salve_spells = {[[%s]], [[%s]]},\n",
                            IS_VALID_SN(pObjIndex->value[4])?  
                        skill_table[pObjIndex->value[4]]->name:"NONE",
                IS_VALID_SN(pObjIndex->value[5])?
                        skill_table[pObjIndex->value[5]]->name:"NONE" );
        ...
        if (pObjIndex->first_extradesc) fprintf( fpout, "\textra_descriptions = {},\n");
        x = 1;
        for ( ed = pObjIndex->first_extradesc; ed; ed = ed->next )
        {
            fprintf( fpout, "\t\textra_description[%d] = { keyword = [[%s]], description = [===[%s]===] },\n",
                x, ed->keyword, strip_cr( ed->description )	);
            x++;
        }

        if (pObjIndex->first_affect) 
        {
            fprintf( fpout, "\taffects = {},\n");
            x = 1;
            char buf[MAX_STRING_LENGTH];
            int xx;
            ...
            for ( paf = pObjIndex->first_affect; paf; paf = paf->next )
            {
                fprintf( fpout, "\t\taffects[%d] = { location = [[%s]], ", x, affect_loc_name( paf->location ));
                switch( paf->location )
                {
                default:
                ...
                case APPLY_EXT_AFFECT:
                    for ( xx = 0; xx < MAX_BITS ; xx++ )
                        if ( xIS_SET( paf->bitvector, xx ) )
                        {
                           sprintf( buf, "%s", a_flags[xx] );
                        }
                    fprintf( fpout, "modifier_name = [[%s]] },\n", buf);
                    break;
                case APPLY_AFFECT:
                    for ( xx = 0; xx < 32 ; xx++ )
                        if ( IS_SET( paf->modifier, 1 << xx ) )
                        {
                            sprintf( buf, "%s", a_flags[xx] );
                        }
                    fprintf( fpout, "modifier_name = [[%s]] },\n", buf);
                ...
                case APPLY_REMOVESPELL:
                    sprintf( buf, "%s", IS_VALID_SN(paf->modifier) ? skill_table[paf->modifier]->name : "unknown" );
                    fprintf( fpout, "modifier_name = [[%s]] },\n", buf);
                    break;
        ...
        if ( pObjIndex->mudprogs )
        {
            ...
            {
                fprintf( fpout, "\t\tmudprogs[%d] = { type = [[%s]], arglist = [[%s]], comlist = [====[%s]====] },\n",
                    x, mprog_type_to_name( mprog->type ), mprog->arglist, strip_cr(mprog->comlist) );
            }
        }
        fprintf( fpout, "\t}, -- end object %d\n", vnum );
    }
    if ( install && vnum < tarea->hi_o_vnum )
      tarea->hi_o_vnum = vnum - 1;
	...
	fprintf( fpout, "\t\trooms[%d] = { ",	vnum				);
	fprintf( fpout, "name = [[%s]],\n",	room->name			);
	fprintf( fpout, "\t\tdescription = [==[%s]==],\n",	strip_cr( room->description )	);
	if ( (room->tele_delay > 0 && room->tele_vnum > 0) || room->tunnel > 0 )
	  fprintf( fpout, "\t\tflags = [[%s]],\n\t\tsectory = %d,\n\t\ttele_delay = %d,\n\t\ttele_vnum = %d,\n\t\ttunnel = %d,\n",
                        flag_string(room->room_flags, r_flags), room->sector_type, room->tele_delay, room->tele_vnum, room->tunnel		);
	else
	  fprintf( fpout, "\t\tflags = %d,\n\t\tsector = %d,\n",	room->room_flags,
					room->sector_type	);
    if (room->first_exit) fprintf( fpout, "\t\texits = {},\n");
    x = 1;
	for ( xit = room->first_exit; xit; xit = xit->next )
	{
        if ( IS_SET(xit->exit_info, EX_PORTAL) ) /* don't fold portals */
            continue;
        fprintf( fpout, "\t\t\texits[%d] = { dir = [[%s]], description = [[%s]], keyword = [[%s]],\n\t\t", 
                x, dir_text[xit->vdir], strip_cr( xit->description ), strip_cr( xit->keyword ) );
        if ( xit->distance > 1 || xit->pull )
            fprintf( fpout, "\t\t\t\tinfo = %d, key = %d, vnum = %d, distance = %d, pulltype = %d, pull = %d},\n",
	     				xit->exit_info & ~EX_BASHED, xit->key, xit->vnum, xit->distance, xit->pulltype,	xit->pull );
        else
            fprintf( fpout, "\t\t\t\tinfo = %d, key = %d, vnum = %d},\n",	
                        xit->exit_info & ~EX_BASHED, xit->key, xit->vnum );
        x++;
    }
    if (room->first_extradesc) fprintf( fpout, "\t\textra_descriptions = {},\n");
    x = 1;
	for ( ed = room->first_extradesc; ed; ed = ed->next )
    {
        fprintf( fpout, "\t\t\textra_descriptions[%d] = { keyword = [[%s]], description = [=====[%s]=====]},\n",
            x, ed->keyword, strip_cr( ed->description ));
        x++;
    }
    ...
    for ( treset = tarea->first_reset; treset; treset = treset->next )
    {
        switch( treset->command ) /* extra arg1 arg2 arg3 */
        {
            ...
            case 'b': case 'B':
            case 'h': case 'H':
                fprintf( fpout, "\t\tresets[%d] = {command = \"%c\", extra = %d, arg1 = %d, arg2 = %d, arg3 = %d},\n", 
                    x, UPPER(treset->command), treset->extra, treset->arg1, treset->arg2, treset->arg3 );
                break;
            case 'g': case 'G':
            case 'r': case 'R':
                fprintf( fpout, "\t\tresets[%d] = {command = \"%c\", extra = %d, arg1 = %d, arg2 = %d},\n", 
                    x, UPPER(treset->command),	treset->extra, treset->arg1, treset->arg2 );
                break;
        }
        x++;
    }
    ...
    for ( vnum = tarea->low_m_vnum; vnum <= tarea->hi_m_vnum; vnum++ )
    {
        ...
        if ( (pShop = pMobIndex->pShop) == NULL )
          continue;
        fprintf( fpout, "\t\tshops[%d] = {keeper = %d, type1 = %d, type2 = %d, type3 = %d, type4 = %d, type5 = %d, profit_buy = %d, profit_sell = %d, ",
            x, pShop->keeper, pShop->buy_type[0], pShop->buy_type[1], pShop->buy_type[2], pShop->buy_type[3], pShop->buy_type[4],
    ...
    for ( vnum = tarea->low_m_vnum; vnum <= tarea->hi_m_vnum; vnum++ )
    {
        ...
        if ( (pRepair = pMobIndex->rShop) == NULL )
            continue;
        fprintf( fpout, "\t\trepairs[%d] = {keeper = %d, type1 = %d, type2 = %d, type3 = %d, profit_fix = %d, type = %d, ",
            x, pRepair->keeper, pRepair->fix_type[0], pRepair->fix_type[1], pRepair->fix_type[2],
    ...
    for ( vnum = tarea->low_m_vnum; vnum <= tarea->hi_m_vnum; vnum++ )
    {
        ...
        if ( !pMobIndex->spec_fun )
            continue;
        fprintf( fpout, "\t\tspecials[%d] = {vnum = %d, spec = [[%s]]},\n",	x, pMobIndex->vnum, lookup_spec( pMobIndex->spec_fun ) );
        x++;
    }
...
void do_foldarea( CHAR_DATA *ch, char *argument )
{
	...
	if ( !str_cmp( tarea->filename, argument ) )
	{
	  ...
	  lua_fold_area( tarea, tarea->filename, FALSE );
	  set_char_color( AT_IMMORT, ch );
	  send_to_char( "Done.\n\r", ch );
	  return;
	}
