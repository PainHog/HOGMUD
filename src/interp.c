...
/*	if ( ch->desc && ch->desc->original )
    ...
    if ( !found )
    {
        char cmdtmp[MAX_INPUT_LENGTH];
        char argtmp[MAX_INPUT_LENGTH];
        char cmdtmp2[MAX_INPUT_LENGTH];
        char argtmp2[MAX_INPUT_LENGTH];
		sprintf(cmdtmp, "%s", command);
		sprintf(argtmp, "%s", argument);
		sprintf(cmdtmp2, "%s", command);
		sprintf(argtmp2, "%s", argument);
        /*
         * something screwy goes on here. the command and argument variables become
         * invalid after the last check_ is called. created temp variables to copy
         * the values into. this seems to work and prevents crash. still not sure
         * what would cause the variables to become invalid. check into it later. -Darwin
         */
        if(!check_skill(ch, command, argument) && !check_alias(ch, command, argument) && !check_social(ch, cmdtmp2, argtmp2))
        {
            EXIT_DATA *pexit;
            /* check for an auto-matic exit command */
            if((pexit = find_door(ch, cmdtmp, TRUE)) != NULL && IS_SET(pexit->exit_info, EX_xAUTO))
            {
...
SOCIALTYPE *find_social( char *command )
{
    ...
    if ( command && (command[0] < 'a' || command[0] > 'z' ))
	hash = 0;
    else
	hash = (command[0] - 'a') + 1;
...
bool check_social( CHAR_DATA *ch, char *command, char *argument )
{
    ...
    {
        if( i == 127 ) /* No more than 128 in array */
            break;
        ...
        if (is_ignoring (victim, ch))    	{
    		if(!IS_IMMORTAL(ch) || get_trust(victim) > get_trust(ch))
    		{
    			...
    			//~ LINK(victim, remfirst, remlast, next_in_room,
    				//~ prev_in_room);
                i++;
                UNLINK( victim, room->first_person, room->last_person, next_in_room, 
                prev_in_room );
            }
    		else
    		{
    ...
    else if( ( ( victim = get_char_room( ch, arg ) ) == NULL )/* && str_cmp( victim->name, arg ) */)
    {
...
/*
 * Pick off one argument from a string and return the rest.
 * Understands double quotes only. 
 * For use with do_pcrename -Darwin
 */
