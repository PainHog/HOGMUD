...
bool is_note_to( CHAR_DATA *ch, NOTE_DATA *pnote )
{
    ...
    if ( (IS_IMMORTAL(ch) && is_name( "immortal", pnote->to_list )) || (IS_IMMORTAL(ch) && is_name( "Immortals", pnote->to_list )) )
	return TRUE;

    if ( (IS_HERO(ch) && is_name( "avatars", pnote->to_list )) || (IS_HERO(ch) && is_name( "Avatars", pnote->to_list )) )
        return TRUE;
...
void do_note( CHAR_DATA *ch, char *arg_passed, bool IS_MAIL )
{
	...
	if ( !can_read( ch, board ) )
	{
	    ...
	    for ( pnote = board->first_note; pnote; pnote = pnote->next )
            {
		vnum++;
		if ( ( (first_list && vnum >= first_list) || !first_list ) && is_note_to( ch, pnote ) )
...
/*	char *pn;*/
	...
	if ( board->postmessg )
	...
	else
 	{
		...
		sprintf( buf, "[INFO] A note has been posted!");
	...
	for ( pnote = board->first_note; pnote; pnote = pnote->next )
	{
	    ...
	    ||  get_trust(ch) >= sysdata.take_others_mail))
	       vnum++;
	    else if (!IS_MAIL && ((is_note_to(ch, pnote))))
               vnum++;
