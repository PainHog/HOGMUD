...
void do_mppardon( CHAR_DATA *ch, char *argument )
{
	...
	if ( xIS_SET(victim->act, PLR_BULLY) )
	{
	    ...
	    send_to_char( "Bully flag removed.\n\r", ch );
	    send_to_char( "Your crime of bullying has been pardoned.\n\r", victim );
	}
    return;
    }
...
void do_mpadvance( CHAR_DATA *ch, char *argument )
{
    ...
    if (victim->class != victim->dualclass)
       victim->exp   = ( ((class_table[victim->class]->exp_base) + (class_table[victim->dualclass]->exp_base)) * 2);
    else
       victim->exp   = ((class_table[victim->class]->exp_base) + (class_table[victim->class]->exp_base));

    if (class_table[victim->class] == class_table[victim->dualclass])
      victim->exp = ( (class_table[victim->class]->exp_base) * 2 );
    if ( (class_table[victim->class] != class_table[victim->dualclass]) && 
       ( (class_table[victim->class]->exp_base) == (class_table[victim->dualclass]->exp_base) ) )
      victim->exp = ( ( (class_table[victim->class]->exp_base) + ( (class_table[victim->dualclass]->exp_base) * .6) ) * 2);
    if ( (class_table[victim->class]->exp_base) > (class_table[victim->dualclass]->exp_base) )
      victim->exp = ( ( (class_table[victim->class]->exp_base) + ( (class_table[victim->dualclass]->exp_base) * .6) ) * 2);
    if ( (class_table[victim->class]->exp_base) < (class_table[victim->dualclass]->exp_base) )
      victim->exp = ( ( (class_table[victim->dualclass]->exp_base) + ( (class_table[victim->class]->exp_base) * .6) ) * 2);
